...
Assert-Contains $powerManager '\.charging\s*=\s*usb_power_present\s*&&\s*board_snapshot\.bat_chg_level\s*==\s*0' 'USB-gated active-low charging interpretation'
...
Assert-Contains $powerManager 'POWER_MANAGER_LOW_BATTERY_SHUTDOWN_MAX_MV\s+3000U' 'critical low-battery shutdown requires product-empty voltage'
