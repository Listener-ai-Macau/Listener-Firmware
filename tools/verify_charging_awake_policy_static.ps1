...
Assert-Contains $powerManager 'bool\s+usb_power_present\s*=\s*board_snapshot\.usb_power_present' 'disabled USB_Det interpreted state'
Assert-Contains $powerManager 'bool\s+charger_active\s*=\s*board_snapshot\.bat_chg_level\s*==\s*0' 'active-low charger indication'
Assert-Contains $powerManager 'bool\s+charge_power_present\s*=\s*usb_power_present\s*\|\|\s*charger_active' 'USB or active charger raw charge-power gate'
...
Assert-Contains $powerManager '\.external_power_present\s*=\s*usb_power_present\s*\|\|\s*charger_active' 'active charging keeps external power true while GPIO7 USB_DET is disabled'
...
Assert-Contains $powerManager 'raw_full\s*=\s*source->bat_std_level\s*==\s*0' 'central filter reads active-low raw full independently of GPIO7'
Assert-Contains $powerManager 'charge_status_present\s*=\s*source->charge_power_present\s*\|\|\s*raw_full' 'BAT_STD full can prove charger status when GPIO7 is low'
...
Assert-Contains $powerManager 'source->charge_full\s*=\s*charge_status_present\s*&&\s*s_charge_full_latched' 'interpreted charge-full uses charger-status debounce latch'
...
Assert-Contains $powerManager 'source->external_power_present\s*=[\s\S]*source->usb_power_present\s*\|\|\s*source->charging\s*\|\|\s*source->charge_full' 'filtered external power follows interpreted USB, active charging, or debounced full'
...
Assert-Contains $powerManager 'GPIO7 USB_DET is disabled[\s\S]*BAT_CHG is the active-charging signal[\s\S]*BAT_STD[\s\S]*charger-standby/full' 'charger status fallbacks are documented next to the source snapshot'
