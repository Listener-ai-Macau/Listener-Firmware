...
Assert-Contains $powerManager 'Active CHG is the only safe immediate[\s\S]*fallback when USB_DET is quiet[\s\S]*bounded window' 'active charging fallback is retained without making STD/full an immediate external-power source'
