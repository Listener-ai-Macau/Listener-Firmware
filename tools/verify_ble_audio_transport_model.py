...
def case_queue_pressure_pause_resume_hysteresis() -> None:
    model = ready_model()
    model.start(106)
    model.pressure_update(queue_depth=46, pool_in_use=20)
    assert model.backpressure_active
    model.pressure_update(queue_depth=38, pool_in_use=20)
    assert model.backpressure_active
    model.pressure_update(queue_depth=30, pool_in_use=20)
