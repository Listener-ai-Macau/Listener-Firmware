...
Assert-Contains -RelativePath "ports/esp32/diag_log_platform/diag_log_flash.c" -Pattern "const uint16_t last_index = \(uint16_t\)\(DIAG_EVENTS_PER_SECTOR - 1U\)" -Description "diag_log startup sector count first checks the final event slot"
Assert-Contains -RelativePath "ports/esp32/diag_log_platform/diag_log_flash.c" -Pattern "while \(low < high\)" -Description "diag_log startup sector count uses binary search instead of a linear full-sector scan"
Assert-NotContains -RelativePath "ports/esp32/diag_log_platform/diag_log_flash.c" -Pattern "for\s*\(\s*uint16_t\s+index\s*=\s*0;\s*index\s*<\s*DIAG_EVENTS_PER_SECTOR;\s*index\+\+\s*\)" -Description "linear retained-event scan on every boot"
