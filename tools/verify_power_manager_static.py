...
def main() -> int:
    ...
    if not re.search(
        ...
        r"charge_status_present\s*=\s*source->charge_power_present\s*\|\|\s*raw_full[\s\S]*"
        r"raw_full\s*&&[\s\S]*"
        r"!raw_charging\s*&&[\s\S]*"
        r"power_manager_charge_full_battery_allowed[\s\S]*"
        r"POWER_MANAGER_CHARGE_FULL_DEBOUNCE_MS[\s\S]*"
        r"source->charge_full\s*=\s*charge_status_present\s*&&\s*s_charge_full_latched[\s\S]*"
        r"source->external_power_present\s*=[\s\S]*source->usb_power_present\s*\|\|\s*source->charging\s*\|\|\s*source->charge_full",
        power_manager,
    ):
        failures.append(
            "components/power_manager/power_manager.c: charge-full must be debounce/near-full guarded while allowing BAT_STD to prove charger status with GPIO7 disabled"
        )
    if not re.search(
