...
def main() -> int:
    ...
    diag_store = read("third_party/denzic-platform/observability/embedded/c/src/denzic_diag_log_store.c")
    ...
    for token in (
        "DENZIC_DIAG_LOG_SOURCE_LAST_MAX_SCANNED_EVENTS 512u",
        "scan_limit = use_source_filter ? DENZIC_DIAG_LOG_SOURCE_LAST_MAX_SCANNED_EVENTS : limit;",
        "scanned < scan_limit && kept < limit",
        "after bounded scan=",
    ):
        require(diag_store, token, "bounded diagnostic source export", failures)
    bounded_dump_start = diag_store.find("static void store_dump_last_filtered")
    bounded_dump_end = diag_store.find("void denzic_diag_log_store_dump_last(", bounded_dump_start)
    bounded_dump = diag_store[bounded_dump_start:bounded_dump_end] if bounded_dump_start >= 0 and bounded_dump_end >= 0 else ""
