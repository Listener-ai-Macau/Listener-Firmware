...
Assert-Contains $bleHid 'raw_full\s*=\s*usb_power_present\s*&&\s*power\.bat_std_level\s*==\s*0' 'charge-full raw pin is active-low and trusted only while USB power is present'
Assert-Contains $bleHid 'ble_hid_battery_estimate_charge_full\([\s\S]*usb_power_present[\s\S]*raw_charging[\s\S]*raw_full[\s\S]*now_ms' 'charge-full uses the debounced USB-gated estimator'
