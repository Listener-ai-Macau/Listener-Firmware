...
Assert-Contains -RelativePath $stream -Pattern "BLE_AUDIO_STREAM_BACKPRESSURE_PAUSE_QUEUE_DEPTH\s+16U" -Description "low-latency queue pause threshold"
Assert-Contains -RelativePath $stream -Pattern "BLE_AUDIO_STREAM_BACKPRESSURE_RESUME_QUEUE_DEPTH\s+6U" -Description "low-latency queue resume threshold"
Assert-Contains -RelativePath $stream -Pattern "BLE_AUDIO_STREAM_BACKPRESSURE_PAUSE_POOL_IN_USE\s+16U" -Description "low-latency pool pause threshold"
Assert-Contains -RelativePath $stream -Pattern "BLE_AUDIO_STREAM_BACKPRESSURE_RESUME_POOL_IN_USE\s+6U" -Description "low-latency pool resume threshold"
Assert-Contains -RelativePath $stream -Pattern "BLE_AUDIO_STREAM_BACKPRESSURE_PAUSE_PERCENT\s+95U" -Description "emergency pause threshold"
Assert-Contains -RelativePath $stream -Pattern "BLE_AUDIO_STREAM_BACKPRESSURE_RESUME_PERCENT\s+70U" -Description "emergency resume threshold"
...
Assert-Contains -RelativePath $stream -Pattern "queue_or_pool_above_latency_window" -Description "backpressure pause has low-latency queue/pool window"
Assert-Contains -RelativePath $stream -Pattern "queue_and_pool_below_latency_window" -Description "backpressure resume has low-latency queue/pool window"
...
Assert-Contains -RelativePath $transportModel -Pattern "case_queue_pressure_pause_resume_hysteresis" -Description "transport model queue pressure hysteresis case"
