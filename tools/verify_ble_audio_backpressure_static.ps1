...
Assert-Contains -RelativePath $stream -Pattern "portMUX_TYPE\s+s_audio_pool_lock\s*=\s*portMUX_INITIALIZER_UNLOCKED" -Description "infallible cross-core audio-pool bookkeeping"
Assert-Contains -RelativePath $stream -Pattern "portENTER_CRITICAL\(&s_audio_pool_lock\)" -Description "atomic audio-pool acquire/release"
Assert-NotContains -RelativePath $stream -Pattern "pool_lock_timeout" -Description "scheduler hiccup misclassified as QueueFull"
Assert-NotContains -RelativePath $stream -Pattern "s_audio_pool_mutex" -Description "fallible audio-pool mutex wait"
$platformTransportContract = "third_party/denzic-platform/audio/embedded/c/include/denzic_audio_transport_v1_generated.h"
Assert-Contains -RelativePath $stream -Pattern "denzic_audio_transport_v1_backpressure_decide\(" -Description "backpressure hysteresis decision delegates to the shared platform core"
Assert-Contains -RelativePath $stream -Pattern "_Static_assert\(BLE_AUDIO_STREAM_BACKPRESSURE_PAUSE_PERCENT == DENZIC_AUDIO_TRANSPORT_V1_BACKPRESSURE_PAUSE_PERCENT" -Description "backpressure pause threshold pinned to the platform contract"
Assert-Contains -RelativePath $stream -Pattern "_Static_assert\(BLE_AUDIO_STREAM_BACKPRESSURE_RESUME_PERCENT == DENZIC_AUDIO_TRANSPORT_V1_BACKPRESSURE_RESUME_PERCENT" -Description "backpressure resume threshold pinned to the platform contract"
Assert-Contains -RelativePath $platformTransportContract -Pattern "DENZIC_AUDIO_TRANSPORT_V1_BACKPRESSURE_PAUSE_PERCENT \(95u\)" -Description "platform contract keeps the 95 percent pause threshold"
Assert-Contains -RelativePath $platformTransportContract -Pattern "DENZIC_AUDIO_TRANSPORT_V1_BACKPRESSURE_RESUME_PERCENT \(70u\)" -Description "platform contract keeps the 70 percent resume threshold"
...
Assert-Contains -RelativePath $stream -Pattern "denzic_audio_transport_v1_pacing_t s_audio_pacing" -Description "BLE audio retains fractional media-clock debt in the shared platform pacing state"
Assert-Contains -RelativePath $stream -Pattern "denzic_audio_transport_v1_pacing_note_pcm_sent\(" -Description "BLE audio media-clock pacing delegates to the shared platform core"
...
$platformLosslessCodec = "third_party/denzic-platform/audio/embedded/c/src/denzic_audio_lossless_v1.c"
$platformLosslessContract = "third_party/denzic-platform/audio/embedded/c/include/denzic_audio_v1_generated.h"
...
Assert-Contains -RelativePath $platformLosslessContract -Pattern "DENZIC_AUDIO_V1_LOSSLESS_RICE_PREDICTOR_FIRST_ORDER\s+\(1u\)" -Description "lossless Rice V2 first-order predictor"
Assert-Contains -RelativePath $platformLosslessContract -Pattern "DENZIC_AUDIO_V1_LOSSLESS_RICE_PREDICTOR_SECOND_ORDER\s+\(2u\)" -Description "lossless Rice V2 second-order predictor"
Assert-Contains -RelativePath $platformLosslessContract -Pattern "DENZIC_AUDIO_V1_LOSSLESS_RICE_PREDICTOR_FOURTH_ORDER\s+\(4u\)" -Description "lossless Rice V3 fourth-order predictor"
Assert-Contains -RelativePath $platformLosslessContract -Pattern "DENZIC_AUDIO_V1_LOSSLESS_RICE_V2_PARAMETER_K_SPAN\s+\(1u\)" -Description "lossless Rice V2 uses a bounded parameter search"
...
Assert-Contains -RelativePath $stream -Pattern "denzic_audio_lossless_v1_encode" -Description "lossless Rice encoder delegates to the shared platform codec"
Assert-Contains -RelativePath $platformLosslessCodec -Pattern "denzic_audio_lossless_v1_encode" -Description "shared platform lossless Rice encoder"
Assert-Contains -RelativePath $platformLosslessCodec -Pattern "denzic_audio_lossless_v1_decode" -Description "shared platform lossless Rice decoder"
...
Assert-Contains -RelativePath $stream -Pattern "BLE_AUDIO_STREAM_COOPERATIVE_IDLE_BLOCK_MS\s+2U" -Description "continuous audio export blocks for a full scheduler interval so IDLE0 can run"
Assert-Contains -RelativePath $stream -Pattern "BLE_AUDIO_STREAM_PREROLL_IDLE_BLOCK_PCM_BYTES\s+7680U" -Description "accelerated preroll has a bounded CPU fairness quantum"
...
Assert-Contains -RelativePath $stream -Pattern "idle_block_boundary_crossed" -Description "lossless preroll periodically enters a real Blocked state"
Assert-NotContains -RelativePath $stream -Pattern "taskYIELD\(\)" -Description "BLE audio never mistakes a same-priority yield for IDLE0 scheduling"
...
Assert-Contains -RelativePath $gap -Pattern "BLE_HID_GAP_ACTIVE_MIN_CE_LEN\s+12U" -Description "active audio requests a long event budget at the host-optimized interval"
Assert-Contains -RelativePath $gap -Pattern "BLE_HID_GAP_ACTIVE_MAX_CE_LEN\s+24U" -Description "active audio allows the controller to retain a long event budget at 15ms"
...
Assert-Contains -RelativePath $stream -Pattern "(?s)s_replay_window->pending\s*&&\s*s_replay_window->pending_session_id\s*==\s*session_id.*?ble_audio_stream_replay_store_packet.*?skip_replay_current_packet\s*=\s*true" -Description "current audio packet retained before replay drain and marked for skip"
Assert-Contains -RelativePath $stream -Pattern "(?s)heap_caps_calloc\(.*?sizeof\(\*s_replay_window\).*?MALLOC_CAP_SPIRAM\s*\|\s*MALLOC_CAP_8BIT\).*?s_replay_window\s*==\s*NULL.*?return\s+ESP_ERR_NO_MEM" -Description "bulk replay history is allocated in PSRAM with fail-closed initialization"
