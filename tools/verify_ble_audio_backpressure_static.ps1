...
Assert-Contains -RelativePath $stream -Pattern "BLE_AUDIO_STREAM_NOTIFY_WINDOW_DEPTH\s+3" -Description "BLE audio notification window depth"
...
Assert-Contains -RelativePath $stream -Pattern "s_notify_tx_inflight" -Description "BLE audio tracks all in-flight notifications"
Assert-Contains -RelativePath $stream -Pattern "ble_audio_stream_note_notify_tx_queued" -Description "BLE audio consumes a window slot before notify"
Assert-Contains -RelativePath $stream -Pattern "ble_audio_stream_complete_notify_tx" -Description "BLE audio returns a window slot only on notify completion"
...
Assert-Contains -RelativePath $stream -Pattern "requires NimBLE notify support for enabled-path mbuf ownership and notify_tx attempt pacing" -Description "notify-disabled build guard explains enabled-path ownership contract"
Assert-NotContains -RelativePath $stream -Pattern "ble_audio_stream_cancel_notify_tx_queued" -Description "manual notify failure credit cancellation"
Assert-NotContains -RelativePath $stream -Pattern "(?s)if\s*\(\s*rc\s*==\s*BLE_HS_ENOMEM\s*\)[\s\S]{0,900}xSemaphoreGive\(s_notify_credit_sem\)" -Description "duplicate ENOMEM notify credit return after GAP notify_tx event"
Assert-NotContains -RelativePath $stream -Pattern '(?s)ESP_LOGW\(\s*TAG,\s*"notify failed:[\s\S]{0,700}xSemaphoreGive\(s_notify_credit_sem\)' -Description "duplicate generic notify-failure credit return after GAP notify_tx event"
...
Assert-Contains -RelativePath $transportModel -Pattern "case_notify_failure_event_owns_credit" -Description "transport model notify failure credit ownership case"
