...
Assert-Contains $deviceSettings 's_settings\.battery_brightness_percent\s*=\s*s_settings\.plugged_brightness_percent' 'battery brightness follows plugged brightness after NVS load and legacy brightness writes'
Assert-Contains $deviceSettings 'next\.battery_brightness_percent\s*=\s*next\.plugged_brightness_percent' 'DEVICE SET persists one Type-controlled brightness profile'
