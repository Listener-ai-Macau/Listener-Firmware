...
Assert-Contains $deviceHeader 'DEVICE_SETTINGS_DEFAULT_STATUS_LED_BRIGHTNESS_PERCENT\s+80U' 'default status LED brightness is 80 percent'
...
Assert-Contains $statusLed 'zone_brightness_peak_normalized=1' 'routine product LEDs normalize each zone peak before applying Type brightness'
Assert-Contains $statusLed 'status_led_normalize_strip_peak_to_type_max' 'status LED normalizes per-strip effect peaks to the Type-defined zone maximum'
Assert-Contains $statusLed 'status_led_normalize_strip_peak_to_type_max\(frame->status, STATUS_LED_STATUS_COUNT\);[\s\S]*status_led_scale_strip_percent\(\s*frame->status' 'status zone peak normalization runs before Type brightness scaling'
Assert-Contains $statusLed 'status_led_normalize_strip_peak_to_type_max\(frame->ec11, STATUS_LED_EC11_COUNT\);[\s\S]*status_led_scale_strip_percent\(\s*frame->ec11' 'EC11 zone peak normalization runs before Type brightness scaling'
