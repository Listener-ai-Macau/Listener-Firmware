...
Assert-Contains $statusLed 'STATUS_LED_CHARGING_BREATH_PERIOD_MS 3600U' 'plugged charging breath uses natural product cadence'
Assert-Contains $statusLed 'STATUS_LED_CHARGING_BREATH_LOW_HOLD_MS 300U' 'plugged charging breath holds a visible near-off valley'
Assert-Contains $statusLed 'STATUS_LED_CHARGING_BREATH_RISE_MS 1050U' 'plugged charging breath rises smoothly'
Assert-Contains $statusLed 'STATUS_LED_CHARGING_BREATH_HIGH_HOLD_MS 80U' 'plugged charging breath only briefly touches peak brightness'
...
Write-Host "PASS: device settings static checks cover firmware command contract, persisted settings, status LED brightness profiles, split low-power idle timeouts, split auto-shutdown timeouts, BLE name source, CMake dependencies, and docs."
