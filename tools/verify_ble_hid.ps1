...
if ([string]::IsNullOrWhiteSpace($Port)) {
    ...
    $checks = @(
        ...
        @($bleHid, 'BLE_HID_USB_READ_LOW_POWER_TIMEOUT_MS 500', "BLE HID slows USB serial polling while idle"),
        @($bleHid, 'ble_hid_low_power_idle_active\(\)[\s\S]*BLE_HID_USB_READ_LOW_POWER_TIMEOUT_MS[\s\S]*BLE_HID_USB_READ_ACTIVE_TIMEOUT_MS', "BLE HID selects low-power USB serial read timeout"),
