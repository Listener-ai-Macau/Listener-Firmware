...
if ([string]::IsNullOrWhiteSpace($Port)) {
    ...
    $checks = @(
        ...
        @($bleHid, 's_ble_connected\s*\?\s*BLE_HID_BATTERY_SAMPLE_INTERVAL_MS\s*:\s*BLE_HID_BATTERY_DISCONNECTED_IDLE_INTERVAL_MS', "BLE HID selects active vs disconnected idle battery sample interval"),
        @($bleHid, '!s_ble_connected\s*&&\s*!force_notify', "BLE HID skips routine HID battery updates while disconnected"),
        @($bleHid, 'watchdog_platform_task_notify_take_low_power\([\s\S]*ble_hid_battery_sample_interval_ms\(\)', "BLE HID battery task uses low-power long wait while disconnected"),
