...
function New-FeatureSnapshot {
    return [ordered]@{
        ...
        key_paths = @(
            ...
            [ordered]@{ path = "components/power_manager/"; purpose = "Low-power state machine, shutdown blockers, PWR_HOLD/GPIO11 active-low hardware shutdown, transition diagnostics, and reset/status diagnostics." },
            [ordered]@{ path = "components/battery_monitor/"; purpose = "Shared battery voltage and level reading for HID and power diagnostics." },
            [ordered]@{ path = "docs/features/low_power_wake_policy.md"; purpose = "Firmware long-idle hardware shutdown contract for active-low PWR_HOLD/GPIO11 and external-power blockers." },
        ...
        hardware_assumptions = @(
            ...
            "V2 long-idle shutdown is firmware-controlled by driving active-low PWR_HOLD/GPIO11 high; real power-off, short-press cold boot, and charging-blocker behavior require hardware-gated validation.",
