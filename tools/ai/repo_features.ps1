...
function New-FeatureSnapshot {
    return [ordered]@{
        ...
        major_features = @(
            ...
            "power_manager low-power state machine for connected idle, disconnected idle, charge-aware automatic hardware-shutdown blocking, and long-idle PWR_HOLD/GPIO46 hardware shutdown with reset/cold-boot diagnostics.",
            ...
            "V2 safety gates keep real PWR_HOLD/GPIO46 power-off validation in hardware-gated workflow evidence and LED calibration commands blocked until VDD_LED sign-off; the N16R8 validation build enables SPH0655 PDM microphone capture on CLK/GPIO48 and DOUT/GPIO47 for A1/A2.",
        ...
        key_paths = @(
            ...
            [ordered]@{ path = "components/power_manager/"; purpose = "Low-power state machine, shutdown blockers, PWR_HOLD/GPIO46 hardware shutdown, transition diagnostics, and reset/status diagnostics." },
            [ordered]@{ path = "components/battery_monitor/"; purpose = "Shared battery voltage and level reading for HID and power diagnostics." },
            [ordered]@{ path = "docs/features/low_power_wake_policy.md"; purpose = "Firmware long-idle hardware shutdown contract for PWR_HOLD/GPIO46 and external-power blockers." },
        ...
        hardware_assumptions = @(
            ...
            "V2 long-idle shutdown is firmware-controlled through PWR_HOLD/GPIO46; real power-off, short-press cold boot, and charging-blocker behavior require hardware-gated validation.",
