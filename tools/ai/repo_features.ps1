...
function New-FeatureSnapshot {
    return [ordered]@{
        ...
        major_features = @(
            ...
            "power_manager low-power state machine for connected idle, disconnected idle, USB/VBUS automatic hardware-shutdown blocking, and long-idle PWR_HOLD/GPIO11 active-high hold/release-low hardware shutdown with reset/cold-boot diagnostics.",
            "V2 board diagnostics for ~BOARD:STATUS and ~LED:STATUS, including USB/charger status, battery ADC, optional absent-safe TPS63020/SY7088 input branch current telemetry rows, LED resource mapping, per-semantic status RGB, idempotent BLE status-window updates, and hardware blocker policy strings.",
        ...
        key_paths = @(
            ...
            [ordered]@{ path = "components/power_manager/"; purpose = "Low-power state machine, shutdown blockers, PWR_HOLD/GPIO11 active-high hold/release-low hardware shutdown, transition diagnostics, and reset/status diagnostics." },
            [ordered]@{ path = "components/battery_monitor/"; purpose = "Shared battery voltage and level reading for HID and power diagnostics." },
            [ordered]@{ path = "docs/features/low_power_wake_policy.md"; purpose = "Firmware long-idle hardware shutdown contract for active-high PWR_HOLD/GPIO11 and USB/VBUS external-power blockers." },
        ...
        hardware_assumptions = @(
            ...
            "V2 long-idle shutdown is firmware-controlled by driving active-high PWR_HOLD/GPIO11 low; real power-off, short-press cold boot, and USB/VBUS blocker behavior require hardware-gated validation.",
