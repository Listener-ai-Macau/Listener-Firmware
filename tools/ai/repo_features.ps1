...
function New-FeatureSnapshot {
    return [ordered]@{
        ...
        major_features = @(
            ...
            "V2 board diagnostics for ~BOARD:STATUS and ~LED:STATUS, including USB/charger status, battery ADC, battery-side TPS63020/SY7088 input branch current telemetry, LED resource mapping, per-semantic status RGB, idempotent BLE status-window updates, and hardware blocker policy strings.",
