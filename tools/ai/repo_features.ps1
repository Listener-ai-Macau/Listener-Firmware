...
function New-FeatureSnapshot {
    return [ordered]@{
        ...
        major_features = @(
            ...
            "diag_log flash ring buffer for boot, BLE, audio, health, error, and operator-enabled input debug events that survive reboot.",
        ...
        key_paths = @(
            ...
            [ordered]@{ path = "components/diag_log/"; purpose = "Diagnostic event schema, ring-buffer API, and default-off input debug event IDs." },
