...
function New-FeatureSnapshot {
    return [ordered]@{
        ...
        major_features = @(
            ...
            "N4 board profile with 4 MB flash, no PSRAM, KEY1-KEY4 logical custom keys on GPIO45/GPIO48/GPIO47/GPIO21, EC11 push recording control on GPIO35, and static checks rejecting stale V2/N16R8 defaults.",
        ...
        key_paths = @(
            ...
            [ordered]@{ path = "docs/features/low_power_wake_policy.md"; purpose = "Firmware wake policy contract for N4 KEY4/GPIO21 deep-sleep wake, EC11/GPIO35 recording-key wake limitation, and production primary voice/wake requirements." },
        ...
        hardware_assumptions = @(
            ...
            "N4 logical custom keys use GPIO45/GPIO48/GPIO47/GPIO21 and fall back to F13/F14/F15/F16; EC11 uses GPIO36/GPIO38/GPIO37/GPIO35.",
