...
foreach ($item in @(
    ...
    @($batteryMonitor, "BATTERY_MONITOR_ADC_SOURCE_IMPEDANCE_NUMERATOR 1010U", "68K/68K battery ADC source-impedance compensation numerator"),
    @($batteryMonitor, "BATTERY_MONITOR_ADC_SOURCE_IMPEDANCE_DENOMINATOR 1000U", "68K/68K battery ADC source-impedance compensation denominator"),
    ...
    @($board, "battery_adc_correction_mv=%d", "battery ADC source-impedance correction diagnostic"),
    @($board, "product_empty_2800mv_full_4200mv_absolute_min_2700mv_adc_source_impedance_x1010", "battery percentage policy output"),
