...
foreach ($item in @(
    ...
    @($keyboard, "EC11 ready: a=gpio42 b=gpio2 key=gpio11", "EC11 V2 ready log"),
    @($voiceKeyInput, 'VOICE_KEY_INPUT_DIRECT_LABEL\s+"ec11_key\.gpio11"', "EC11 recording diagnostic label"),
...
foreach ($item in @(
    @($boardPins, "BOARD_PINS_EC11_KEY_IO\s+\(GPIO_NUM_(18|35)\)", "stale EC11 key GPIO"),
    @($boardPins, "BOARD_PINS_PWR_HOLD_IO\s+\(GPIO_NUM_11\)", "stale PWR_HOLD GPIO11"),
