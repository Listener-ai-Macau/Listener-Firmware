...
foreach ($item in @(
    ...
    @($audioCapture, "AUDIO_CAPTURE_PDM_SOFTWARE_GAIN_MAX_NUM\s+32U", "V2 PDM weak-input guarded calibration ceiling"),
    @($audioCapture, "AUDIO_CAPTURE_PDM_SOFTWARE_GAIN_TARGET_PEAK\s+16000U", "V2 PDM calibration headroom target"),
    @($audioCapture, "AUDIO_CAPTURE_PDM_SOFTWARE_GAIN_RECOVERY_Q8\s+64U", "V2 PDM bounded calibration recovery"),
    ...
    @($audioCapture, "sw_gain_max=%u sw_gain_target_peak=%u", "SPH0655 PDM calibration init log"),
    ...
    @($board, "\.usb_power_present\s*=\s*usb_serial_jtag_sof_active\s*&&\s*charger\.vin_present", "USB host power requires SOF plus exclusive charger VIN"),
    @($board, "const bool charging = chg && !std", "charger decode charging is exclusive of full"),
    @($board, "const bool standby_full = std && !chg", "charger decode full is exclusive of charging"),
    @($board, "\.vin_present = charging \|\| standby_full", "charger VIN is exclusive charging or exclusive full"),
    ...
    @($batteryMonitor, "BATTERY_MONITOR_ADC_DMM_TRIM_MIN_MV \(-300\)", "battery ADC DMM trim lower bound"),
    @($batteryMonitor, "BATTERY_MONITOR_ADC_DMM_TRIM_MAX_MV 300", "battery ADC DMM trim upper bound"),
    @($batteryMonitor, 'BATTERY_MONITOR_NVS_ADC_TRIM_KEY "adc_trim_mv"', "battery ADC DMM trim NVS key"),
    @($batteryMonitor, "battery_monitor_calibrate_adc_trim_from_dmm_mv", "battery ADC DMM trim calibration API"),
    @($batteryMonitor, "BATTERY_MONITOR_ABSOLUTE_MIN_MV 2700U", "absolute battery danger marker"),
    @($batteryMonitor, "BATTERY_MONITOR_EMPTY_MV 2850U", "protected product empty battery voltage"),
    @($batteryMonitor, "BATTERY_MONITOR_FULL_MV 4150U", "full battery voltage"),
    ...
    @($board, "~BATTERY:CAL:DMM <mV>", "battery DMM trim calibration command help"),
    @($board, "battery_adc_driver_mv=%d", "battery driver ADC pad millivolt diagnostic"),
    @($board, "battery_adc_raw_mv=%d", "battery raw ADC pad millivolt diagnostic"),
    @($board, "battery_adc_trim_mv=%d", "battery ADC DMM trim diagnostic"),
    @($board, "battery_adc_correction_mv=%d", "legacy battery ADC correction alias diagnostic"),
    @($board, "product_empty_2850mv_full_4150mv_absolute_min_2700mv_adc_dmm_trim_nvs", "battery percentage policy output"),
    @($board, "v2_optional_current_telemetry_not_populated_battery_adc_only_no_power_decisions", "current telemetry absent policy"),
    ...
    @($currentTelemetryTool, "allowed_gpios = @\(-1\)", "current telemetry optional GPIO parser rejects GPIO9/GPIO10"),
    @($currentTelemetryTool, "current_telemetry_not_populated", "current telemetry not-populated self-test"),
    ...
    @($currentTelemetryTool, "pwr_hold_gpio=9", "current telemetry self-test PWR_HOLD GPIO9"),
    ...
    @($currentTelemetryTool, "v2_gpio9_power_latch_runtime_low_drive_high_for_hardware_shutdown", "current telemetry self-test PWR_HOLD runtime-low policy"),
...
foreach ($item in @(
    ...
    @($boardPins, "BOARD_PINS_BAT_V_ADC_IO\s+\(GPIO_NUM_8\)", "stale battery ADC GPIO8"),
    @($boardPins, "BOARD_PINS_PWR_HOLD_IO\s+\(GPIO_NUM_(11|46)\)", "stale PWR_HOLD GPIO"),
    ...
    @($keyboard, "gpio35|gpio45|gpio48\.f14|gpio47\.f15|gpio21\.f16|pwr_hold_gpio11_shutdown_separate", "N4 or stale keyboard diagnostic labels"),
