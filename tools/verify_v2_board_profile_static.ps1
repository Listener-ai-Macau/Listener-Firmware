...
foreach ($item in @(
    ...
    @($currentTelemetryTool, "v2_gpio11_power_latch_runtime_low_release_high_for_hardware_shutdown", "current telemetry self-test PWR_HOLD runtime-low policy"),
    ...
    @($powerManager, "board_set_power_hold_enabled\(false\)", "power manager releases PWR_HOLD high for shutdown"),
