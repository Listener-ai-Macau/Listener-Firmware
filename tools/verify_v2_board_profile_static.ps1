...
foreach ($item in @(
    ...
    @($currentTelemetryTool, "pwr_hold_configured=0", "current telemetry self-test PWR_HOLD runtime output-drive disabled state"),
    @($currentTelemetryTool, "v2_gpio11_power_latch_input_pulldown_runtime_drive_high_for_hardware_shutdown", "current telemetry self-test PWR_HOLD input-pulldown policy"),
...
foreach ($item in @(
    ...
    @($powerManager, "KEY4/GPIO21|EC11-KEY/GPIO35|wake_policy|wake_gpio|PWR_HOLD/GPIO46|release-high|hold-low", "stale wake diagnostics"),
