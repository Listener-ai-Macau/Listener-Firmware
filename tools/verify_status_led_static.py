...
def main() -> int:
    ...
    if re.search(r"\.mem_block_symbols\s*=\s*64\b", status_led):
        failures.append("status_led.c: RMT mem_block_symbols=64 consumes two ESP32-S3 RMT blocks per strip and leaves fewer than four TX channels")
    if not re.search(r"\.mem_block_symbols\s*=\s*SOC_RMT_MEM_WORDS_PER_CHANNEL\b", status_led):
        failures.append("status_led.c: RMT strip channels must use SOC_RMT_MEM_WORDS_PER_CHANNEL so all four V2 LED zones can initialize")
