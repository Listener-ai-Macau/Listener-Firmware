...
function New-SelfTestTranscript {
    return @(
        ...
        "> ~POWER:STATUS",
        "~POWER:STATUS state=CONNECTED_IDLE blockers=0x00000080 blocker_names=external_power shutdown_blockers=0x00000080 shutdown_blocker_names=external_power idle_ms=54000 user_idle_ms=54000 radio_idle_ms=54000 ble_connected=1 automatic_shutdown_enabled=1 automatic_shutdown_blocked_by_external_power=0 external_power_present=1 usb_power_present=1 usb_serial_jtag_sof_active=1 charger_active=0 charge_power_present=1 charging=0 charge_full=1 charge_full_latched=1 charge_full_candidate_ms=34000 charge_full_debounce_ms=10000 charge_full_min_mv=4050 charge_full_min_percent=88 plugged_low_power_enabled=1 low_power_idle_allowed=1 power_input_wake_configured=1 power_input_irq_armed=0 usb_det_level=low usb_det_adc_valid=0 usb_det_adc_mv=0 usb_det_mismatch=0 bat_chg_level=high bat_std_level=low pwr_hold_level=low usb_det_policy=v2_gpio7_usb_det_disabled_highz_usb_sof_and_charger_status charger_polarity=v2_gpio14_chg_gpio21_std_active_low battery_mv=4084 battery_level=92 battery_valid=1 last_shutdown_reason=none last_shutdown_idle_ms=0 last_shutdown_blockers=0x00000000 guard=1 audio_idle_ms=5000 audio_idle_power_save=1 audio_idle_blockers=0x00000000 low_power_idle_ms=60000 plugged_low_power_idle_ms=60000 battery_low_power_idle_ms=60000 connected_idle_ms=60000 disconnected_idle_ms=60000 hardware_shutdown_ms=1800000 plugged_auto_shutdown_ms=0 battery_auto_shutdown_ms=1800000 pwr_hold_gpio=9 pwr_hold_level=low pwr_hold_configured=1 pwr_hold_policy=v2_gpio9_power_latch_runtime_low_drive_high_for_hardware_shutdown voice_key_gpio=18"
    )
}
...
function Test-ChargingAwakeTranscript {
    ...
    Assert-Contains -Text $text -Pattern '(?m)^~BOARD:STATUS\s+.*pwr_hold_gpio=9.*pwr_hold_configured=1.*runtime_low_drive_high' -Description "PWR_HOLD/GPIO9 is actively driven low during runtime and driven high for hardware shutdown"
    ...
    if ($ExpectExternalPower.IsPresent) {
        ...
        if ((Get-StatusField -Line $lastPower -Name "usb_power_present") -ne "0") {
            Add-Error "expected usb_power_present=0 in final ~POWER:STATUS because GPIO7 USB_Det is disabled"
        } else {
            Add-Evidence "PASS: final power status reports usb_power_present=0 with disabled GPIO7 USB_Det"
        }
        if ((Get-StatusField -Line $lastPower -Name "charge_power_present") -ne "1") {
            Add-Error "expected charge_power_present=1 in final ~POWER:STATUS"
        } else {
            Add-Evidence "PASS: final power status reports charge_power_present=1 from charger status"
        }
