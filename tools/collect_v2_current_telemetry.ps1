<#
.SYNOPSIS
Collect optional V2 current-rail diagnostics and low-power status evidence.

.DESCRIPTION
Reads firmware diagnostics and writes a focused report for the two optional
current telemetry slots. The latest V2 N16R8 board revision does not populate
the current-sense chips:
  - TPS63020_I_ADC / not populated
  - SY7088_I_ADC / not populated

GPIO9 is PWR_HOLD and GPIO10 is BAT_V_ADC on the latest board; neither may be
reported as current telemetry.
...
$expectedSensors = @(
    [ordered]@{
        ...
        net = "TPS63020_I_ADC"
        allowed_gpios = @(-1)
        role = "not populated on current V2 board; GPIO10 is BAT_V_ADC"
    },
    [ordered]@{
        branch = "SY7088_input_branch"
        net = "SY7088_I_ADC"
        allowed_gpios = @(-1)
        role = "not populated on current V2 board; GPIO9 is PWR_HOLD"
    }
)
...
function New-SelfTestLines {
    return @(
        "~BOARD:STATUS profile=voice-keyboard-v2-n16r8 battery_gpio=10 battery_mv=4012 battery_valid=1",
        "~BOARD:POWER branch=TPS63020_input_branch present=0 gpio=-1 raw_adc=0 adc_mv=0 adc_calibrated=0 sample_count=0 calibration_status=current_telemetry_not_populated current_model=""not_populated"" current_calibrated=0 current_ma_valid=0 estimated_input_current_ma=0 battery_side_mv=0 battery_voltage_source=""BAT_V_ADC/GPIO10 68K/68K midpoint, VBAT~=2*ADC"" power_mw_valid=0 estimated_input_power_mw=0 result=ESP_ERR_NOT_SUPPORTED policy=v2_optional_current_telemetry_not_populated_battery_adc_only_no_power_decisions",
        "~BOARD:POWER branch=SY7088_input_branch present=0 gpio=-1 raw_adc=0 adc_mv=0 adc_calibrated=0 sample_count=0 calibration_status=current_telemetry_not_populated current_model=""not_populated"" current_calibrated=0 current_ma_valid=0 estimated_input_current_ma=0 battery_side_mv=0 battery_voltage_source=""BAT_V_ADC/GPIO10 68K/68K midpoint, VBAT~=2*ADC"" power_mw_valid=0 estimated_input_power_mw=0 result=ESP_ERR_NOT_SUPPORTED policy=v2_optional_current_telemetry_not_populated_battery_adc_only_no_power_decisions",
        "~POWER:STATUS state=CONNECTED_IDLE blockers=0x00000000 blocker_names=none shutdown_blockers=0x00000000 shutdown_blocker_names=none idle_ms=100 user_idle_ms=100 radio_idle_ms=100 ble_connected=1 automatic_shutdown_enabled=1 automatic_shutdown_blocked_by_external_power=0 external_power_present=0 usb_power_present=0 usb_serial_jtag_sof_active=0 charger_active=0 charge_power_present=0 charging=0 charge_full=0 charge_full_latched=0 charge_full_candidate_ms=0 charge_full_debounce_ms=10000 charge_full_min_mv=4050 charge_full_min_percent=88 plugged_low_power_enabled=1 low_power_idle_allowed=1 power_input_wake_configured=1 power_input_irq_armed=0 usb_det_level=low usb_det_adc_valid=0 usb_det_adc_mv=0 usb_det_mismatch=0 bat_chg_level=high bat_std_level=high pwr_hold_level=low usb_det_policy=v2_gpio7_usb_det_disabled_highz_usb_sof_and_charger_status charger_polarity=v2_gpio14_chg_gpio21_std_active_low battery_mv=3988 battery_level=82 battery_valid=1 last_shutdown_reason=manual_command last_shutdown_idle_ms=100 last_shutdown_blockers=0x00000000 guard=1 audio_idle_ms=60000 audio_idle_power_save=1 audio_idle_blockers=0x00000000 low_power_idle_ms=60000 plugged_low_power_idle_ms=60000 battery_low_power_idle_ms=60000 connected_idle_ms=60000 disconnected_idle_ms=60000 hardware_shutdown_ms=1800000 plugged_auto_shutdown_ms=0 battery_auto_shutdown_ms=1800000 pwr_hold_gpio=9 pwr_hold_level=low pwr_hold_configured=1 pwr_hold_policy=v2_gpio9_power_latch_runtime_low_drive_high_for_hardware_shutdown voice_key_gpio=18 hardware_shutdown_user_action=""short-press hardware power key for cold boot after PWR_HOLD/GPIO9 drive-high shutdown"""
    )
}
...
try {
    ...
    if ($summary.all_expected_present -and @($summary.gpio_mismatches).Count -eq 0 -and $powerStatusOk) {
        if ($SelfTest) {
            Write-Host "PASS: V2 optional current telemetry parser self-test passed."
        } else {
            Write-Host "PASS: V2 optional current telemetry captured."
        }
    } else {
