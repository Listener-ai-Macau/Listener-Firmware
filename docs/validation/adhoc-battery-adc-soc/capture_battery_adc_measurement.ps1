...
function Show-TopmostMeasurementPrompt {
    ...
    $form.Text = "oai3 / Listener硬件电池ADC测量"
    $form.TopMost = $true
    ...
    $label.Text = "agent=oai3。现在请用万用表测 Listener 硬件 BAT_V_ADC/GPIO10 的 68K/68K 分压点到 GND 的电压。不是电脑界面，不要按 KEY1-KEY4，也不要按 EC11。测到后在下面输入电压，单位 V，例如 2.083。固件会同时抓取 raw ADC、内阻/源阻抗补偿后的 ADC、电池电压和电量。"
    ...
    Add-Line "PROMPT agent=oai3 ui=Listener硬件BAT_V_ADC/GPIO10分压点 action=measure_adc_voltage"
...
try {
    ...
    if ([double]::TryParse($normalized, [Globalization.NumberStyles]::Float, [Globalization.CultureInfo]::InvariantCulture, [ref]$parsedVoltage)) {
        ...
        if ($null -ne $firmwareAdcMv) {
            ...
            Add-Line ("COMPARE dmm_adc_mv={0} firmware_adc_mv={1} raw_adc_mv={2} correction_mv={3} delta_mv={4} battery_mv={5} battery_level={6}" -f $dmmAdcMv, $firmwareAdcMv, $firmwareRawAdcMv, $firmwareCorrectionMv, $deltaMv, $batteryMv, $batteryLevel)
        } else {
