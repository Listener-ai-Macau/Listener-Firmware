...
| Custom BLE HID fallback keys | Logical custom keys emit stable `KEY1`/`KEY2`/`KEY3`/`KEY4` firmware events. Current N4 GPIO45/GPIO48/GPIO47/GPIO21 fall back to safe non-text BLE HID usages F13/F14/F15/F16 until Listener-Type consumes custom key actions. | `components/keyboard/`, `components/hid_keyboard/`, `ports/esp32/ble_hid*`, `ports/esp32/board_pins/` |
| Recording control key | EC11 push/GPIO35 controls recording: single click starts or stops after the double-click window, while USB `VREC:` commands remain available for desktop/product validation. | `components/voice_recording_control/`, `ports/esp32/voice_key_input/` |
