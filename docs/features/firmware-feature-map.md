...
| Low power | `power_manager` preserves battery-only connected/disconnected idle power reduction, keeps USB/charging/full external-power sessions awake, and keeps board-level `PWR_HOLD/GPIO11` LOW during boot/runtime, releasing it HIGH for long-idle hardware shutdown. `~POWER:STATUS` reports shutdown blockers, reset/cold-boot-oriented diagnostics, PWR_HOLD state, and `DIAG_POWER_USB_DETECT` / `DIAG_POWER_CHARGE_STATE` / `DIAG_POWER_HOLD_STATE` transitions; real power-off proof belongs to the hardware validation gate. | `components/power_manager/`, `docs/features/low_power_wake_policy.md`, `tools/verify_power_manager_static.py`, `tools/verify_charging_awake_policy_static.ps1` |
