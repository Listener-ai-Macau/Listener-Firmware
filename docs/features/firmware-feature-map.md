...
| Low power | GPIO7 USB_DET is disabled/high-Z and is diagnostics-only. Computer USB is inferred from USB Serial/JTAG SOF; charger/external-power state comes from active-low `BAT_CHG/GPIO14` and `BAT_STD/GPIO21` with debounce and near-full guards. `power_manager` preserves connected/disconnected idle power reduction through device-settings thresholds, lets plugged idle stay active when plugged low-power is disabled, keeps idle microphone/I2S sleep independent from external power, keeps restrained PWR/BLE status visible in idle, arms BAT_CHG/BAT_STD wake only while battery idle, and uses the active plugged/battery shutdown timeouts. Real power-off still goes through `PWR_HOLD/GPIO9` drive-high with bounded readback/removal windows and hardware validation evidence. | `components/power_manager/`, `components/device_settings/`, `components/status_led/`, `docs/features/low_power_wake_policy.md`, `tools/verify_power_manager_static.py`, `tools/verify_charging_awake_policy_static.ps1`, `tools/verify_device_settings_static.ps1`, `tools/verify_diagnostic_log_coverage.ps1` |
