...
| Low power | `power_manager` preserves connected/disconnected idle power reduction, blocks automatic shutdown on USB/charging/full external power, and uses board-level `PWR_HOLD/GPIO11` LOW for long-idle hardware shutdown. `~POWER:STATUS` reports shutdown blockers, reset/cold-boot-oriented diagnostics, and PWR_HOLD state; real power-off proof belongs to the hardware validation gate. | `components/power_manager/`, `docs/features/low_power_wake_policy.md`, `tools/verify_power_manager_static.py`, `tools/verify_charging_awake_policy_static.ps1` |
