...
| Battery and factory readiness | Battery ADC status, USB serial text commands, readiness flags, capability strings, and a verifiable factory firmware package are exposed for production bring-up. | `ports/esp32/ble_hid*`, `ports/esp32/board_pins/`, `tools/package_factory_firmware.ps1`, `tools/verify_factory_firmware_package.ps1`, `!docs/features/factory_firmware_readiness.md` |
| Low power | `power_manager` preserves connected/disconnected idle power reduction, blocks automatic shutdown on USB/charging/full external power, and uses board-level `PWR_HOLD/GPIO46` LOW for long-idle hardware shutdown. `~POWER:STATUS` reports shutdown blockers, reset/cold-boot-oriented diagnostics, PWR_HOLD state, and `DIAG_POWER_USB_DETECT` / `DIAG_POWER_CHARGE_STATE` / `DIAG_POWER_HOLD_STATE` transitions; real power-off proof belongs to the hardware validation gate. | `components/power_manager/`, `docs/features/low_power_wake_policy.md`, `tools/verify_power_manager_static.py`, `tools/verify_charging_awake_policy_static.ps1` |
