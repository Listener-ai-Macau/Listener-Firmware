...
| Battery and factory readiness | Battery ADC status uses the protected product range `2800mV=0%` and `4200mV=100%`, with `2700mV` reserved as an absolute danger marker. BLE HID Battery Service reports the live ADC-derived level, periodically refreshes the host value, forces a final pre-disconnect battery refresh before hardware shutdown, and clamps to `100%` only when charge power is present and the charger full pin is asserted. USB serial text commands, readiness flags, capability strings, and a verifiable factory firmware package are exposed for production bring-up. | `components/battery_monitor/`, `ports/esp32/ble_hid*`, `ports/esp32/board_pins/`, `tools/package_factory_firmware.ps1`, `tools/verify_factory_firmware_package.ps1`, `tools/verify_ble_battery_service_static.ps1`, `docs/features/factory_firmware_readiness.md` |
| Device settings | `device_settings` persists Type-facing board settings: plugged brightness, battery brightness, low-power idle timeout, plugged low-power enable switch, battery-only auto-shutdown timeout, and BLE name. Fresh-device defaults are plugged brightness 80%, battery brightness 50%, low-power idle 1 minute, plugged low-power enabled, auto-shutdown 30 minutes, and BLE name `listener`; user/NVS settings override those defaults. `~DEVICE:SET auto_shutdown_minutes=off` or `auto_shutdown_ms=0` disables inactivity-triggered hardware shutdown for bench/debug sessions without removing the manual shutdown test command. `~DEVICE:SETTINGS` reports the effective settings and `~DEVICE:SET ...` updates them with validation. The legacy `~LED:BRIGHTNESS` command remains compatible by writing both brightness profiles. BLE name changes are persisted and marked pending until the next safe advertising/reconnect/reboot path. | `components/device_settings/`, `protocols/listener_device/`, `components/status_led/`, `components/power_manager/`, `tools/verify_device_settings_static.ps1` |
| Low power | `power_manager` preserves connected/disconnected idle power reduction through the device-settings low-power timeout, holds GPIO7 USB_Det disabled/high-Z because that pad is unreliable at runtime, and derives external-power state from the charger pins instead: active-low `BAT_CHG` proves active charging, while `BAT_STD` proves charger standby/full after debounce and a near-full battery guard. It uses the persisted plugged low-power switch to keep externally powered idle in `ACTIVE` when disabled, lets the idle microphone/I2S path sleep even while plugged after the audio idle threshold, turns routine status LEDs off in connected/disconnected idle, keeps BLE disconnect/reconnect churn from reopening a high-power ACTIVE window once user idle has elapsed, uses the device-settings battery-only shutdown timeout, actively drives board-level `PWR_HOLD/GPIO9` LOW as the first `app_main()` boot action and during runtime, guards it back to LOW outside hardware shutdown, and drives it HIGH only for hardware shutdown with a bounded long-pulse sequence: at least 10 seconds high, up to 15 seconds for high readback, then a bounded 10 second power-removal observation. Automatic failed PWR_HOLD readback or still-powered shutdown fallback restores runtime LOW, records diagnostics, and enters a bounded shutdown-failure retry cooldown while staying low-power/LED-off instead of reopening BLE reconnect every timeout; manual `~POWER:SHUTDOWN` or `~POWER:TEST:SHUTDOWN` remains the true serial-triggered hardware shutdown path for bring-up. Charging or charge-full status blocks long-idle and critical-low-battery automatic shutdown. `~POWER:STATUS` reports shutdown blockers, `audio_idle_power_save`, audio-idle blockers, effective `low_power_idle_ms`, `plugged_low_power_enabled`, `low_power_idle_allowed`, `automatic_shutdown_enabled`, `hardware_shutdown_ms`, shutdown-failure retry cooldown, reset/cold-boot-oriented diagnostics, PWR_HOLD state, disabled GPIO7 diagnostics, and `DIAG_POWER_USB_DETECT` / `DIAG_POWER_CHARGE_STATE` / `DIAG_POWER_HOLD_STATE` transitions. Unplugged low-power validation must dump/decode flash diag after reconnect and use `DIAG_LED_POWER_INPUT` / `DIAG_LED_VISUAL_STATE` / `DIAG_LED_OUTPUT_STATE` plus power events to reconstruct LED-off/resume and PWR yellow/green behavior; status LED battery color uses a display-latched battery level while raw battery readings stay in diagnostics and safety paths. Real power-off proof belongs to the hardware validation gate. | `components/power_manager/`, `components/device_settings/`, `components/status_led/`, `docs/features/low_power_wake_policy.md`, `tools/verify_power_manager_static.py`, `tools/verify_charging_awake_policy_static.ps1`, `tools/verify_device_settings_static.ps1`, `tools/verify_diagnostic_log_coverage.ps1` |
