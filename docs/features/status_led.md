...
User brightness scales the whole routine effect envelope before the profile cap is applied. A 50% user brightness setting turns a 1-100% external-power breath into roughly 1-50%, rather than clipping all samples above 50% into a flat line. The charging breath includes a short 1% valley hold, smooth inhale, and longer smooth exhale, so the visibly dark low point is reached without making the effect feel like a hard blink.
