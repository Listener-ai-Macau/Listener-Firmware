...
User brightness scales the whole routine effect envelope before the profile cap is applied. A 50% user brightness setting keeps the external-power breath's 1% valley near-off and scales the 100% peak to roughly 50%, rather than clipping all samples above 50% into a flat line.
