...
`~LED:STATUS detail=power` reports `external_power_source=usb_det|charger_status|none`. `usb_det` is the normal VBUS divider path; `charger_status` is a visual fallback for boards where `BAT_CHG_IO` proves the charger is active while `USB_DET/GPIO7` reads low. This prevents a plugged board from falling back to battery-only PWR/BLE behavior while charging. `~LED:STATUS detail=rgb`, `detail=rgb_ec11`, `detail=rgb_key`, and `detail=rgb_edge` print the last transmitted RGB frame for all four LED zones.
...
- EC11 knob and edge/frame LEDs are independent accent surfaces. Standard profile uses restrained accents for recording, processing, OK, pairing, and reconnect; ambient adds calm low-blue idle accents. These accents never set status semantics, never recolor key LEDs, and are suppressed below low battery except safety/error/test paths.
...
- `~LED:PREVIEW <ready|pairing|reconnect|capture|desktop_mic|recording_processing|rec_not_available|processing|ok|low_battery|critical_battery|charging|full|sleep|clear>`
