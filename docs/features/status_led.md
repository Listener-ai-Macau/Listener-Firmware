...
- Recording is a gold voice-reactive semantic state on `LED3=REC` only; it does not borrow key LEDs. Firmware keeps a low baseline breath, exposes `rec_level`, and adds a visible VU brightness envelope from recent PCM so speech makes `REC` visibly brighter while keeping the user brightness cap as the hard maximum.
