...
- `LED7`-`LED10`: local key feedback on `PWM_RGB_KEY`, `BOARD_PINS_RGB_KEY_IO`, GPIO13.
- `LED11`-`LED16`: outer edge group on `PWM_RGB_Edge`, `BOARD_PINS_RGB_EDGE_IO`, GPIO4.
...
WS2812 output uses ESP-IDF RMT at 10 MHz with an 800 kHz WS2812 encoder and a 50 us reset latch. The status LED task refreshes every 50 ms and sends at most three short strip frames, so BLE, OTA, keyboard scan, power manager, and diagnostics are not blocked by software bit-banging.
...
- `~LED:TEST:MAP <status|key|edge|all>`
...
Real V2 hardware validation still has to record the actual RGB color order and physical LED order with photos or video. The firmware default color order is `GRB` for all three strips.
