...
- Recording is a gold voice-reactive semantic state on status `LED3=REC`; it does not borrow key LEDs and does not clear independent `PWR`/`BLE` status. Firmware keeps a low baseline breath, exposes `rec_level`, and adds a visible VU brightness envelope from recent PCM so speech makes `REC` visibly brighter while keeping the user brightness cap as the hard maximum. The EC11 ring may show a low gold dual-dot motion and the edge/frame may show gold side accents as non-semantic support. Starting a new recording clears any stale `OK` completion window and retryable recording warning in the same LED-state update, so `LED5=OK`/`LED6=WARN` cannot linger into active capture.
- Processing uses a saturated purple breath on `AI` once the desktop host starts streaming/ASR processing; long processing settles to a calmer breath. The EC11 ring may show a violet orbit and the edge/frame may show a violet comet, but those accents do not change the status rail semantics. Starting a new processing window clears stale `OK` and retryable AI/OTA warning windows before the first processing frame, so REC+AI overlap is isolated to `LED3` and `LED4` while `LED1=PWR` and `LED2=BLE` keep their own state unless power/BLE state actually changes.
- Firmware recording transfer does not animate `AI` by itself. The desktop host starts processing with `VREC:PROCESSING:START`, can clear it with `VREC:PROCESSING:STOP`, and reports completion with `VREC:PROCESSING:DONE`; `DONE` turns `AI` off and flashes green `OK`.
