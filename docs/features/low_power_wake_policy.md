...
- `~POWER:STATUS` reports `wake_policy=key4_only`, `wake_capable_keys=KEY4/GPIO21`, `wake_gpio_mask`, `wake_key_gpio`, `voice_key_gpio`, `voice_key_rtc_capable`, `voice_key_deep_sleep_wake`, `voice_key_limitation`, `wake_user_action`, thresholds, blockers, battery, and the last sleep/wake state.
...
- Automatic overnight deep sleep keeps the 15 minute default threshold (`CONFIG_POWER_MANAGER_OVERNIGHT_SLEEP_MS=900000`) and uses only RTC-capable wake GPIOs.
