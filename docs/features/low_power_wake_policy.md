...
Current V2/N16R8 firmware uses `components/power_manager` for runtime idle power reduction and long-idle hardware shutdown. The long-idle path no longer uses ESP32-S3 Deep Sleep. Instead, battery-only long idle enters `POWER_MANAGER_STATE_HARDWARE_SHUTDOWN`, records diagnostics, prepares BLE/audio/LED shutdown, and releases `PWR_HOLD/GPIO11` LOW through the board abstraction. `EC11-KEY_IO/GPIO18` remains the active runtime recording key.
...
- Automatic long-idle hardware shutdown keeps the 30 minute V2 default threshold (`CONFIG_POWER_MANAGER_HARDWARE_SHUTDOWN_MS=1800000`) and uses only the board-level `PWR_HOLD/GPIO11` active-high hold latch released LOW for shutdown. No ESP sleep source is configured. If the board remains powered after latch release, firmware keeps `PWR_HOLD/GPIO11` released LOW, feeds the watchdog, and suppresses BLE advertising restarts so the host does not see a reconnect loop while the device is supposed to be off.
...
Real confirmation that `PWR_HOLD/GPIO11` LOW fully removes power, that short press cold-boots the device afterward, and that USB/VBUS blocks automatic shutdown belongs to the hardware validation gate. Static firmware checks must not claim real power-off success.
