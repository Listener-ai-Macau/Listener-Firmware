...
- Battery percentage uses the product-protection range `3000mV=0%` and `4200mV=100%`; `2700mV` is treated only as an absolute danger marker, not as usable empty capacity. Battery-only operation at the critical percentage threshold forces hardware shutdown through the same `PWR_HOLD/GPIO11` release path before the cell is driven into the irreversible-damage region.
