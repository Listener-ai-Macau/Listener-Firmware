...
#define BLE_HID_GAP_ACTIVE_FALLBACK_ITVL_MAX 12U
...
/* CE units are 0.625 ms. Windows ThroughputOptimized commonly selects a
 * 15 ms interval. Accept 7.5-15 ms as the active range and keep the event
 * budget within that same range so an existing 15 ms link is used directly
 * instead of interrupting OTA with rejected 7.5 ms-only requests. */
...
/* Windows ThroughputOptimized primes 251 octets / 965 us. Type aligns OTA data
 * to two link packets at that measured envelope before releasing its 15-ms lease. */
...
static bool s_active_preferred_interval_rejected = false;
...
static TickType_t s_audio_data_length_request_started_tick = 0;
...
static uint16_t s_audio_data_length_max_tx_time_us = 0;
static uint16_t s_audio_data_length_max_rx_time_us = 0;
...
static bool ble_hid_gap_data_length_ready_for_phy(
    uint16_t max_tx_octets,
    uint16_t max_tx_time_us,
    uint8_t tx_phy,
    uint8_t rx_phy)
{
    const bool full_2m_link =
        tx_phy == BLE_HCI_LE_PHY_2M && rx_phy == BLE_HCI_LE_PHY_2M;
    const uint16_t minimum_packet_time_us = full_2m_link
        ? BLE_HID_GAP_AUDIO_DATA_LEN_2M_PACKET_TIME_US
        : BLE_HID_GAP_AUDIO_DATA_LEN_TIME_US;
    return max_tx_octets >= BLE_HID_GAP_AUDIO_DATA_LEN_OCTETS &&
           max_tx_time_us >= minimum_packet_time_us;
}
...
static uint16_t ble_hid_gap_active_interval_max(void)
{
    portENTER_CRITICAL(&s_ble_gap_state_lock);
    bool preferred_rejected = s_active_preferred_interval_rejected;
    portEXIT_CRITICAL(&s_ble_gap_state_lock);
    return preferred_rejected
        ? BLE_HID_GAP_ACTIVE_FALLBACK_ITVL_MAX
...
static void ble_hid_gap_confirm_conn_param_update(uint16_t conn_handle)
{
    ...
    s_active_preferred_interval_rejected = false;
    portEXIT_CRITICAL(&s_ble_gap_state_lock);
...
static void ble_hid_gap_defer_conn_param_retry_after_rejection(uint16_t conn_handle, int status)
{
    TickType_t retry_not_before_tick =
        xTaskGetTickCount() + pdMS_TO_TICKS(BLE_HID_GAP_CONN_PARAM_REJECT_BACKOFF_MS);
    portENTER_CRITICAL(&s_ble_gap_state_lock);
    const bool active_request =
        s_last_conn_param_mode == (uint32_t)BLE_HID_CONN_PARAM_MODE_ACTIVE;
    s_last_conn_param_mode = 0;
    s_conn_param_retry_not_before_tick = retry_not_before_tick;
    s_conn_param_request_pending_until_tick = 0;
    if (active_request) {
        s_active_preferred_interval_rejected = true;
    }
    portEXIT_CRITICAL(&s_ble_gap_state_lock);

    ESP_LOGW(
        TAG,
        "connection parameter update rejected; deferring shared retry for %u ms conn=%u status=%d active_fallback=%u",
        (unsigned)BLE_HID_GAP_CONN_PARAM_REJECT_BACKOFF_MS,
        conn_handle,
        status,
        active_request ? 1U : 0U);
}
...
typedef struct {
    ...
    uint16_t audio_data_length_max_tx_time_us;
    uint16_t audio_data_length_max_rx_time_us;
...
static ble_hid_gap_connection_snapshot_t ble_hid_gap_connection_snapshot(void)
{
    ...
    ble_hid_gap_connection_snapshot_t snapshot = {
        ...
        .audio_data_length_max_tx_time_us = s_audio_data_length_max_tx_time_us,
        .audio_data_length_max_rx_time_us = s_audio_data_length_max_rx_time_us,
...
void ble_hid_gap_print_status(void)
{
    ...
    if (conn.connected && conn.conn_handle != BLE_HS_CONN_HANDLE_NONE) {
        ...
        active_applied = descriptor_valid &&
                         ...
                         desc.conn_itvl <= ble_hid_gap_active_interval_max() &&
                         desc.conn_latency == BLE_HID_GAP_ACTIVE_LATENCY;
    }

    printf(
        "~BLE:STATUS connected=%u secure=%u conn_handle=%u descriptor_valid=%u descriptor_rc=%d interval_units=%u interval_ms_x100=%u latency=%u supervision_timeout_units=%u supervision_timeout_ms=%u active_required=%u active_applied=%u e11r=%u lossless_rice=%u lossless_rice_version=%u dle_pending=%u dle_ready=%u dle_tx_octets=%u dle_rx_octets=%u dle_tx_time_us=%u dle_rx_time_us=%u phy_tx=%u phy_rx=%u active_ce_min=%u active_ce_max=%u last_conn_param_mode=%u\n",
        ...
        (unsigned)conn.audio_data_length_max_tx_time_us,
        (unsigned)conn.audio_data_length_max_rx_time_us,
...
static void ble_hid_gap_set_connection_state(bool connected, uint16_t conn_handle)
{
    ...
    s_audio_data_length_request_started_tick = 0;
    s_audio_data_length_ready = false;
    s_audio_data_length_max_tx_octets = 0;
    s_audio_data_length_max_rx_octets = 0;
    s_audio_data_length_max_tx_time_us = 0;
    s_audio_data_length_max_rx_time_us = 0;
    s_audio_tx_phy = 0;
    s_audio_rx_phy = 0;
    if (!connected) {
...
{
    portENTER_CRITICAL(&s_ble_gap_state_lock);
    const bool allowed = denzic_ble_pairing_v1_advertising_allowed_during_bond_delete(
        s_recovery_bond_delete_pending || s_recovery_bond_delete_in_progress,
...
#define BLE_HID_GAP_RECOVERY_PAIRING_WINDOW_MS ((int64_t)DENZIC_BLE_PAIRING_V1_RECOVERY_PAIRING_WINDOW_MS)
#define BLE_HID_GAP_RECOVERY_SWIFT_PAIR_PROMPT_MS ((int64_t)DENZIC_BLE_PAIRING_V1_RECOVERY_SWIFT_PAIR_PROMPT_WINDOW_MS)
#define BLE_HID_GAP_FIRST_PAIRING_WINDOW_MS ((int64_t)DENZIC_BLE_PAIRING_V1_FIRST_PAIRING_WINDOW_MS)
#define BLE_HID_GAP_SWIFT_PAIR_ADV_MIN_RESTART_MS ((int64_t)DENZIC_BLE_PAIRING_V1_SWIFT_PAIR_ADV_MIN_RESTART_MS)
#define BLE_HID_GAP_RECOVERY_BOND_DELETE_WAIT_MS DENZIC_BLE_PAIRING_V1_RECOVERY_BOND_DELETE_WAIT_MS
...
static int64_t s_ec11_recovery_accepted_at_us;
static bool s_ec11_recovery_generated;
...
static void ble_hid_gap_log_ec11_recovery_timing(const char *phase, bool complete)
{
    ...
    const int64_t elapsed_ms = elapsed_us / 1000LL;
    portENTER_CRITICAL(&s_ble_gap_state_lock);
    if (phase != NULL && strcmp(phase, "control_dispatch") == 0) {
        s_ec11_recovery_control_dispatch_ms = elapsed_ms;
    } else if (phase != NULL && strcmp(phase, "terminate_requested") == 0) {
...
void ble_hid_gap_note_ec11_recovery_accepted(uint64_t accepted_at_us, bool generated)
{
    ...
    portENTER_CRITICAL(&s_ble_gap_state_lock);
    s_ec11_recovery_accepted_at_us = (int64_t)accepted_at_us;
    s_ec11_recovery_generated = generated;
    s_ec11_recovery_control_dispatch_ms = -1;
    s_ec11_recovery_terminate_requested_ms = -1;
    s_ec11_recovery_disconnect_confirmed_ms = -1;
    s_ec11_recovery_advertising_command_accepted_ms = -1;
...
static int64_t ble_hid_gap_window_remaining_ms(
    ...
    int64_t window_ms)
{
    return denzic_ble_pairing_v1_window_remaining_ms(
        opened_at_ms,
        window_ms,
        ble_hid_gap_now_ms());
}
...
static int64_t ble_hid_gap_recovery_swift_pair_prompt_remaining_ms(void)
{
    ...
    const int64_t remaining_ms = ble_hid_gap_window_remaining_ms(
        s_recovery_pairing_window_opened_at_ms,
        BLE_HID_GAP_RECOVERY_SWIFT_PAIR_PROMPT_MS);
    switch (denzic_ble_pairing_v1_evaluate_swift_pair_prompt(
        s_recovery_suppress_swift_pair_prompt,
        s_recovery_swift_pair_consumed,
        BLE_HID_GAP_RECOVERY_SWIFT_PAIR_PROMPT_MS,
        remaining_ms)) {
    case DENZIC_BLE_PAIRING_V1_SWIFT_PAIR_PROMPT_SUPPRESSED:
        ...
        return 0;
    case DENZIC_BLE_PAIRING_V1_SWIFT_PAIR_PROMPT_CONSUMED:
        return 0;
    case DENZIC_BLE_PAIRING_V1_SWIFT_PAIR_PROMPT_DISABLED:
        s_recovery_swift_pair_consumed = true;
        ESP_LOGI(TAG, "recovery: Swift Pair prompt disabled by build config; continuing normal pairable advertising");
        return 0;
    case DENZIC_BLE_PAIRING_V1_SWIFT_PAIR_PROMPT_EXPIRED:
        s_recovery_swift_pair_consumed = true;
...
{
    return denzic_ble_pairing_v1_swift_pair_adv_duration_ms(remaining_ms);
}
...
static void ble_hid_gap_begin_recovery_pairing_window(
...
static void ble_hid_gap_activate_recovery_pairing_window(
    bool type_controlled,
    bool suppress_swift_pair_prompt)
{
...
{
    const bool window_open = ble_hid_gap_recovery_pairing_window_open();
    const bool waiting_for_disconnect = s_recovery_waiting_for_disconnect;
    struct ble_gap_conn_desc desc = {0};
    int rc = BLE_HS_ENOTCONN;
    if (window_open && !waiting_for_disconnect) {
        rc = ble_gap_conn_find(conn_handle, &desc);
    }

    if (denzic_ble_pairing_v1_window_close_on_type_audio_ready(
            window_open,
            waiting_for_disconnect,
            rc == 0,
            rc == 0 && desc.sec_state.encrypted,
            rc == 0 && desc.sec_state.bonded) ==
...
static void ble_hid_gap_note_secure_connection(uint16_t conn_handle, const char *reason)
{
    ...
    if (denzic_ble_pairing_v1_window_close_on_secure(
    ...
    portENTER_CRITICAL(&s_ble_gap_state_lock);
    TickType_t ota_fast_until = s_ota_fast_link_expected_until_tick;
    bool ota_fast_expected = ota_fast_until != 0 &&
        !ble_hid_gap_tick_reached(now, ota_fast_until);
    if (ota_fast_until != 0 && !ota_fast_expected) {
        s_ota_fast_link_expected_until_tick = 0;
    }
    portEXIT_CRITICAL(&s_ble_gap_state_lock);
    if (ota_fast_expected) {
        esp_err_t ret = ble_hid_gap_request_active_connection();
        ESP_LOGI(
            TAG,
            "OTA reconnect requested full-speed link immediately after security: conn=%u ret=%s",
            conn_handle,
            esp_err_to_name(ret));
    }
}
...
static esp_err_t ble_hid_gap_request_audio_data_length(const char *policy)
{
    ...
    if (!conn.secure_connected) {
        ESP_LOGD(
            TAG,
            "%s data length update deferred until encrypted link: conn=%u",
            policy,
            conn.conn_handle);
        return ESP_ERR_INVALID_STATE;
    }
    ...
    TickType_t pending_started_tick = s_audio_data_length_request_started_tick;
    TickType_t now = xTaskGetTickCount();
    bool pending_expired = pending && pending_started_tick != 0 &&
        (TickType_t)(now - pending_started_tick) >=
            pdMS_TO_TICKS(BLE_HID_GAP_AUDIO_DATA_LEN_REQUEST_TIMEOUT_MS);
    if (pending_expired) {
        s_audio_data_length_request_pending = false;
        s_audio_data_length_request_started_tick = 0;
        pending = false;
    ...
    if (!ready && !pending) {
        ...
        s_audio_data_length_request_started_tick = now;
    }
    portEXIT_CRITICAL(&s_ble_gap_state_lock);
    if (ready) {
        ESP_LOGD(
    ...
    if (pending) {
        ESP_LOGD(
            TAG,
            "%s data length update already pending: conn=%u",
            policy,
            conn.conn_handle);
        return ESP_OK;
    }
    if (pending_expired) {
        ESP_LOGW(
            TAG,
            "%s data length completion timed out after %u ms; retrying conn=%u",
            policy,
            (unsigned)BLE_HID_GAP_AUDIO_DATA_LEN_REQUEST_TIMEOUT_MS,
            conn.conn_handle);
    }
...
static void ble_hid_gap_ota_delayed_advertising_task(void *arg)
{
    (void)arg;
    vTaskDelay(pdMS_TO_TICKS(BLE_HID_GAP_OTA_RECONNECT_RADIO_QUIET_MS));
    esp_err_t ret = ble_hid_gap_start_advertising();
    if (ret == ESP_OK) {
        ESP_LOGI(
            TAG,
            "OTA reconnect radio quiet completed; advertising restarted after %u ms",
            (unsigned)BLE_HID_GAP_OTA_RECONNECT_RADIO_QUIET_MS);
    } else {
        ESP_LOGE(
            TAG,
            "OTA reconnect advertising restart failed after radio quiet: %s",
            esp_err_to_name(ret));
        status_led_set_error(
            STATUS_LED_ERROR_DOMAIN_BLE,
            STATUS_LED_ERROR_HARD,
            "ble_ota_reconnect_adv_restart_failed");
    }
    vTaskDelete(NULL);
}

static bool ble_hid_gap_schedule_ota_delayed_advertising(void)
{
    BaseType_t started = xTaskCreate(
        ble_hid_gap_ota_delayed_advertising_task,
        "ble_ota_adv_wait",
        ...
        NULL);
    if (started != pdPASS) {
        ESP_LOGE(TAG, "OTA reconnect radio-quiet task allocation failed");
        return false;
    }
    ESP_LOGI(
        TAG,
        "OTA reconnect holding radio quiet for %u ms before advertising",
        (unsigned)BLE_HID_GAP_OTA_RECONNECT_RADIO_QUIET_MS);
    return true;
}
...
static void ble_hid_gap_handle_disconnect(uint16_t conn_handle, int reason, const char *source)
{
    ...
    if (s_recovery_waiting_for_disconnect) {
        if (!recovery_bond_delete_active) {
            ESP_LOGI(TAG,
     ...
     */
    const uint8_t disconnect_classification =
        denzic_ble_pairing_v1_classify_disconnect(reason, false);
    ...
    if (disconnect_recovery_pairing_window &&
        denzic_ble_pairing_v1_rotate_before_advertising(s_native_recovery_identity_rotate_pending)) {
    ...
    portENTER_CRITICAL(&s_ble_gap_state_lock);
    bool ota_radio_quiet = s_ota_reconnect_radio_quiet_pending &&
        s_ota_reconnect_radio_quiet_conn_handle == conn_handle;
    if (ota_radio_quiet) {
        s_ota_reconnect_radio_quiet_pending = false;
        s_ota_reconnect_radio_quiet_conn_handle = BLE_HS_CONN_HANDLE_NONE;
    }
    portEXIT_CRITICAL(&s_ble_gap_state_lock);
    if (ota_radio_quiet && ble_hid_gap_schedule_ota_delayed_advertising()) {
        return;
    }
...
nimble_hid_gap_event(struct ble_gap_event *event, void *arg)
{
    ...
    case BLE_GAP_EVENT_CONN_UPDATE:
        ...
        if (event->conn_update.status == BLE_HS_HCI_ERR(BLE_ERR_DIFF_TRANS_COLL)) {
            ...
            ble_hid_gap_defer_conn_param_retry_after_rejection(
    ...
    case BLE_GAP_EVENT_PHY_UPDATE_COMPLETE:
        ...
        if (event->phy_updated.status == 0) {
            ...
            if (s_ble_gap_connected &&
                ...
                s_audio_data_length_ready = ble_hid_gap_data_length_ready_for_phy(
                    s_audio_data_length_max_tx_octets,
                    s_audio_data_length_max_tx_time_us,
                    s_audio_tx_phy,
                    s_audio_rx_phy);
                if (s_audio_data_length_ready) {
                    /* DATA_LEN_CHG can arrive before PHY_UPDATE_COMPLETE. Once
                     * the PHY-specific full-packet time is satisfied, a repeated
                     * HCI request may return EALREADY without another event. */
                    s_audio_data_length_request_pending = false;
                    s_audio_data_length_request_started_tick = 0;
                }
            }
    ...
    case BLE_GAP_EVENT_SUBSCRIBE:
        if (s_recovery_waiting_for_disconnect &&
            event->subscribe.conn_handle == s_ble_gap_conn_handle) {
    ...
    case BLE_GAP_EVENT_DATA_LEN_CHG:
    {
        ...
        bool restore_full_data_length = false;
        ...
        if (s_ble_gap_connected &&
            ...
            s_audio_data_length_max_tx_time_us = event->data_len_chg.max_tx_time;
            s_audio_data_length_max_rx_time_us = event->data_len_chg.max_rx_time;
            s_audio_data_length_ready = ble_hid_gap_data_length_ready_for_phy(
                s_audio_data_length_max_tx_octets,
                s_audio_data_length_max_tx_time_us,
                s_audio_tx_phy,
                s_audio_rx_phy);
            restore_full_data_length =
                s_ble_gap_secure_connected && !s_audio_data_length_ready;
        }
        portEXIT_CRITICAL(&s_ble_gap_state_lock);
        ...
        if (restore_full_data_length) {
            esp_err_t schedule_ret = ble_hid_gap_schedule_active_connection_with_delay(
                0,
                "data length downgraded");
            ESP_LOGW(
                TAG,
                "data length below full-speed target; restore scheduled: conn=%u tx_octets=%u tx_time_us=%u ret=%s",
                applied_conn_handle,
                event->data_len_chg.max_tx_octets,
                event->data_len_chg.max_tx_time,
                esp_err_to_name(schedule_ret));
        }
        return 0;
    }
    ...
    case BLE_GAP_EVENT_ENC_CHANGE:
        ...
        if (event->enc_change.status == 0) {
            ...
            if (denzic_ble_pairing_v1_security_failure_action(
                    ble_hid_gap_recovery_pairing_window_open()) ==
...
esp_err_t esp_hid_ble_gap_adv_start(void)
{
    ...
    if (rc == 0) {
        ...
        if (bonded_peer_count > 0) {
            ...
            start_directed = denzic_ble_pairing_v1_directed_reconnect_allowed(
                s_directed_adv_pending,
...
static void ble_hid_gap_notify_recovery_bond_delete_disconnect(void)
{
    ...
    if (s_recovery_bond_delete_pending || s_recovery_bond_delete_in_progress) {
        task_handle = s_recovery_bond_delete_task_handle;
    }
    portEXIT_CRITICAL(&s_ble_gap_state_lock);
...
static void ble_hid_gap_recovery_bond_delete_task(void *arg)
{
    ...
    ble_hid_gap_log_ec11_recovery_timing("disconnect_confirmed", false);
    ...
    bool force_fresh_native_identity)
{
    ...
    bool refresh_pairing_window =
        denzic_ble_pairing_v1_refresh_existing_pairing_window(
            ble_hid_gap_recovery_pairing_window_open(),
            bonded_peer_count,
            s_ble_gap_connected,
...
bool ble_hid_gap_active_connection_applied(void)
{
    ...
    return conn.connected && conn.conn_handle != BLE_HS_CONN_HANDLE_NONE &&
           ble_hid_gap_conn_desc_matches_params(
               conn.conn_handle,
               BLE_HID_GAP_ACTIVE_ITVL_MIN,
               ble_hid_gap_active_interval_max(),
               BLE_HID_GAP_ACTIVE_LATENCY);
}
...
bool ble_hid_gap_ota_connection_ready(void)
{
    ...
    if (!conn.connected || !conn.secure_connected ||
        conn.conn_handle == BLE_HS_CONN_HANDLE_NONE) {
        return false;
    }

    struct ble_gap_conn_desc desc;
    if (ble_gap_conn_find(conn.conn_handle, &desc) != 0) {
        return false;
    }

    /* The full host window is safe only after the complete fast-link contract
     * is observable. Keep the bounded window while promotion is still in flight. */
    return desc.conn_latency == BLE_HID_GAP_ACTIVE_LATENCY &&
           desc.conn_itvl >= BLE_HID_GAP_ACTIVE_ITVL_MIN &&
...
static void ble_hid_gap_ota_reconnect_task(void *arg)
{
    ...
    if (!conn.connected || conn.conn_handle != requested_conn_handle) {
        ...
        portENTER_CRITICAL(&s_ble_gap_state_lock);
        s_ota_reconnect_radio_quiet_pending = true;
        s_ota_reconnect_radio_quiet_conn_handle = conn.conn_handle;
        portEXIT_CRITICAL(&s_ble_gap_state_lock);
        rc = ble_gap_terminate(conn.conn_handle, BLE_ERR_REM_USER_CONN_TERM);
        if (rc != 0) {
            portENTER_CRITICAL(&s_ble_gap_state_lock);
...
esp_err_t ble_hid_gap_schedule_ota_reconnect(void)
{
    ...
    if (!already_pending) {
        ...
        s_ota_fast_link_expected_until_tick =
