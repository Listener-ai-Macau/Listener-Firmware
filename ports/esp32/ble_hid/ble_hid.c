...
static void ble_hid_log_dis_gatt_state(void)
{
    ...
    ESP_LOGI(TAG,
             "DIS GATT state: svc_rc=%d svc_handle=%u model_rc=%d model_def=%u model_val=%u serial_rc=%d serial_def=%u serial_val=%u firmware_rc=%d firmware_def=%u firmware_val=%u hardware_rc=%d hardware_def=%u hardware_val=%u software_rc=%d software_def=%u software_val=%u manufacturer_rc=%d manufacturer_def=%u manufacturer_val=%u pnp_rc=%d pnp_def=%u pnp_val=%u",
             svc_rc,
             service_handle,
             model_rc,
             model_def_handle,
