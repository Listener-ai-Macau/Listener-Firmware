...
static esp_err_t keyboard_ec11_start(void)
{
    ...
    ESP_LOGI(
        TAG,
        "EC11 ready: a=gpio42 b=gpio2 key=gpio18 key_policy=gpio18_recording_key_pwr_hold_gpio11_shutdown_separate decoder=interrupt_quadrature direction_policy=clockwise_increases_volume_brightness detent_state=0x%02x idle_poll_ms=%d queue_depth=%d",
