...
esp_err_t device_settings_init(void)
{
    ...
    if (ret == ESP_OK) {
        ...
        ESP_LOGI(
            ...
            " plugged_low_power_enabled=%u auto_shutdown_ms=%" PRIu32
...
static void device_settings_print_status(const char *result)
{
    ...
    printf(
        ...
        " loaded_from_nvs=%u external_power_present=%u usb_power_present=%u charger_active=%u"
        " charge_power_present=%u charging=%u charge_full=%u usb_det_adc_valid=%u usb_det_adc_mv=%d usb_det_mismatch=%u"
...
esp_err_t device_settings_consume_control_command(const char *line)
{
    ...
    if (strcmp(command, "HELP") == 0 || strcmp(command, "?") == 0) {
        printf("~DEVICE:HELP commands=SETTINGS,STATUS,SET,RESET keys=plugged_brightness,battery_brightness,low_power_idle_ms,low_power_idle_minutes,plugged_low_power_enabled,auto_shutdown_ms,auto_shutdown_minutes,ble_name,knob_rotation\n");
        fflush(stdout);
        return ESP_OK;
    }
