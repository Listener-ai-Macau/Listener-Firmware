...
static void hid_keyboard_translate_ascii(uint8_t *report_buffer, char input_char)
{
    ...
    if (input_char >= 'A' && input_char <= 'Z') {
        report_buffer[0] = HID_KEYBOARD_MODIFIER_LEFT_SHIFT;
        report_buffer[2] = (uint8_t)(4 + ((input_char - 'A')));
        return;
    }
    ...
    KEY_CASE('?', HID_KEYBOARD_MODIFIER_LEFT_SHIFT, USB_HID_FORWARD_SLASH);
    KEY_CASE('/', 0, USB_HID_FORWARD_SLASH);
    KEY_CASE('\\', 0, USB_HID_BACK_SLASH);
    KEY_CASE('|', HID_KEYBOARD_MODIFIER_LEFT_SHIFT, USB_HID_BACK_SLASH);
    KEY_CASE(',', 0, USB_HID_COMMA);
    KEY_CASE('<', HID_KEYBOARD_MODIFIER_LEFT_SHIFT, USB_HID_COMMA);
    KEY_CASE('>', HID_KEYBOARD_MODIFIER_LEFT_SHIFT, USB_HID_DOT);
    KEY_CASE('@', HID_KEYBOARD_MODIFIER_LEFT_SHIFT, 31);
    KEY_CASE('!', HID_KEYBOARD_MODIFIER_LEFT_SHIFT, 30);
    KEY_CASE('#', HID_KEYBOARD_MODIFIER_LEFT_SHIFT, 32);
    KEY_CASE('$', HID_KEYBOARD_MODIFIER_LEFT_SHIFT, 33);
    KEY_CASE('%', HID_KEYBOARD_MODIFIER_LEFT_SHIFT, 34);
    KEY_CASE('^', HID_KEYBOARD_MODIFIER_LEFT_SHIFT, 35);
    KEY_CASE('&', HID_KEYBOARD_MODIFIER_LEFT_SHIFT, 36);
    KEY_CASE('*', HID_KEYBOARD_MODIFIER_LEFT_SHIFT, 37);
    KEY_CASE('(', HID_KEYBOARD_MODIFIER_LEFT_SHIFT, 38);
    KEY_CASE(')', HID_KEYBOARD_MODIFIER_LEFT_SHIFT, 39);
    KEY_CASE('-', 0, 0x2D);
    KEY_CASE('_', HID_KEYBOARD_MODIFIER_LEFT_SHIFT, 0x2D);
    KEY_CASE('=', 0, 0x2E);
    KEY_CASE('+', HID_KEYBOARD_MODIFIER_LEFT_SHIFT, 0x2E);
    KEY_CASE(8, 0, 0x2A);
    KEY_CASE('\t', 0, 0x2B);
    KEY_CASE(';', 0, 0x33);
    KEY_CASE(':', HID_KEYBOARD_MODIFIER_LEFT_SHIFT, 0x33);
    KEY_CASE('\'', 0, 0x34);
    KEY_CASE('"', HID_KEYBOARD_MODIFIER_LEFT_SHIFT, 0x34);
    KEY_CASE('`', 0, 0x35);
    KEY_CASE('~', HID_KEYBOARD_MODIFIER_LEFT_SHIFT, 0x35);
    KEY_CASE('[', 0, 0x2F);
    KEY_CASE(']', 0, 0x30);
    KEY_CASE('{', HID_KEYBOARD_MODIFIER_LEFT_SHIFT, 0x2F);
    KEY_CASE('}', HID_KEYBOARD_MODIFIER_LEFT_SHIFT, 0x30);
    default:
        report_buffer[0] = 0;
        report_buffer[2] = 0;
        break;
...
{
    return hid_keyboard_send_usage_with_modifier(usage, 0, hid_device);
}
...
esp_err_t hid_keyboard_send_usage_with_modifier(uint8_t usage, uint8_t modifier, esp_hidd_dev_t *hid_device)
{
    ...
    ESP_LOGI(
        TAG,
        "send_usage usage=0x%02X modifier=0x%02X connected=%s",
        usage,
        modifier,
        connected ? "yes" : "no");
    ...
    ESP_LOGI(TAG, "send_usage done usage=0x%02X modifier=0x%02X", usage, modifier);
    return ESP_OK;
}
