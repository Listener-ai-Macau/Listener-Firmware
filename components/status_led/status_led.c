...
#define STATUS_LED_PHYSICAL_ROUTE_COUNT 2
#define STATUS_LED_MAIN_STATUS_OFFSET 0U
#define STATUS_LED_MAIN_EC11_OFFSET (STATUS_LED_MAIN_STATUS_OFFSET + STATUS_LED_STATUS_COUNT)
#define STATUS_LED_MAIN_KEY_OFFSET (STATUS_LED_MAIN_EC11_OFFSET + STATUS_LED_EC11_COUNT)
#define STATUS_LED_MAIN_COUNT (STATUS_LED_MAIN_KEY_OFFSET + STATUS_LED_KEY_COUNT)
#define STATUS_LED_MAX_STRIP_COUNT STATUS_LED_MAIN_COUNT

_Static_assert(STATUS_LED_MAIN_COUNT == 22U, "V2.2 main RGB chain must contain 22 pixels");
_Static_assert(BOARD_PINS_RGB_MAIN_IO != BOARD_PINS_RGB_EDGE_IO,
               "V2.2 RGB routes must use distinct GPIOs");
...
#define STATUS_LED_PROCESSING_STALE_TIMEOUT_MS 5000U
...
#define STATUS_LED_STRIP_MASK_MAIN \
    (STATUS_LED_STRIP_MASK_STATUS | STATUS_LED_STRIP_MASK_EC11 | STATUS_LED_STRIP_MASK_KEY)
...
typedef struct {
    ...
    bool boot_ble_ready_notified;
    bool boot_ble_ready_seen;
...
static status_led_strip_t s_strips[STATUS_LED_STRIP_COUNT] = {
    {
        .name = "main",
        .gpio = BOARD_PINS_RGB_MAIN_IO,
        .led_count = STATUS_LED_MAIN_COUNT,
        .tail_guard_pixels = STATUS_LED_STATUS_TAIL_GUARD_PIXELS,
        .color_order = STATUS_LED_STATUS_DEFAULT_COLOR_ORDER,
        // V2.2 daisy-chains status, EC11 and key pixels on PWM_RGB/GPIO1.
        // Keep the composite chain on RMT DMA; low-power final latch frames
        // temporarily transmit without DMA.
        .prefer_dma = true,
    },
    {
        .name = "ec11",
        ...
        // Logical zone only. Its pixels are packed into the main backend.
    },
    {
        .name = "key",
        .gpio = GPIO_NUM_NC,
...
static bool status_led_boot_feedback_active_locked(uint32_t now_ms);
static void status_led_apply_boot_feedback_locked(
    ...
    status_led_power_color_class_t power_class)
{
    ...
    if (status_led_boot_feedback_active_locked(now_ms)) {
...
static void status_led_log_visual_state_event_locked(
    ...
    bool force)
{
    ...
    if (!force &&
        s_state.visual_log_initialized &&
...
{
    status_led_log_visual_state_event_locked(frame, now_ms, false);
}
...
static void status_led_pack_main_chain(
    const status_led_frame_t *frame,
    status_led_rgb_t main_pixels[STATUS_LED_MAIN_COUNT])
{
    memcpy(&main_pixels[STATUS_LED_MAIN_STATUS_OFFSET], frame->status, sizeof(frame->status));
    /* V2.2 routes EC11 and KEY from the far end of their former standalone
     * strips. Keep logical animation/key indexes stable and reverse only the
     * physical serialization inside the shared GPIO1 chain. */
    for (size_t index = 0; index < STATUS_LED_EC11_COUNT; ++index) {
        main_pixels[STATUS_LED_MAIN_EC11_OFFSET + index] =
            frame->ec11[STATUS_LED_EC11_COUNT - 1U - index];
    }
    for (size_t index = 0; index < STATUS_LED_KEY_COUNT; ++index) {
        main_pixels[STATUS_LED_MAIN_KEY_OFFSET + index] =
...
static void status_led_note_main_transmitted(uint32_t now_ms)
{
    const bool suspended =
        !status_led_strip_backend_available(s_strips[STATUS_LED_STRIP_STATUS].backend);
    s_strip_last_tx_ms[STATUS_LED_STRIP_STATUS] = now_ms;
    s_strip_last_tx_ms[STATUS_LED_STRIP_EC11] = now_ms;
    s_strip_last_tx_ms[STATUS_LED_STRIP_KEY] = now_ms;
    s_strip_transport_suspended[STATUS_LED_STRIP_STATUS] = suspended;
    s_strip_transport_suspended[STATUS_LED_STRIP_EC11] = suspended;
    s_strip_transport_suspended[STATUS_LED_STRIP_KEY] = suspended;
}
...
{
    uint8_t mask = 0U;
    if (s_strip_transport_suspended[STATUS_LED_STRIP_STATUS]) {
        mask = (uint8_t)(mask | STATUS_LED_STRIP_MASK_MAIN);
    }
    if (s_strip_transport_suspended[STATUS_LED_STRIP_EDGE]) {
...
static void status_led_suspend_quiet_idle_transports(bool low_power_active, uint32_t now_ms)
{
    ...
    static const status_led_strip_id_t physical_strips[] = {
        STATUS_LED_STRIP_STATUS,
        STATUS_LED_STRIP_EDGE,
    };
    for (size_t physical_index = 0;
         physical_index < sizeof(physical_strips) / sizeof(physical_strips[0]);
         ++physical_index) {
        status_led_strip_id_t index = physical_strips[physical_index];
        if (s_strip_transport_suspended[index] || s_strip_last_tx_ms[index] == 0U) {
            continue;
        }
        ...
        if (index == STATUS_LED_STRIP_STATUS) {
            ...
            s_strip_transport_suspended[STATUS_LED_STRIP_KEY] = true;
        }
    }
}
    ...
    bool key_dark_latch_pending_tx)
{
    ...
    (void)key_dark_latch_pending_tx;
    ...
    if ((strip_mask & STATUS_LED_STRIP_MASK_MAIN) != 0U) {
        status_led_rgb_t main_pixels[STATUS_LED_MAIN_COUNT] = {0};
        ...
        if (status_transmitted) {
            status_led_note_main_transmitted(tx_ms);
        } else {
            esp_err_t failure = last_status_ret == ESP_OK ? ESP_FAIL : last_status_ret;
            ESP_LOGW(TAG,
                     "LED strip tx failed strip=main ret=%s force_non_dma=%u",
                     esp_err_to_name(failure),
                     force_non_dma ? 1U : 0U);
            diag_log(DIAG_SRC_STATUS_LED, DIAG_LED_OUTPUT_FAIL, DIAG_SEV_WARN,
                     (uint32_t)BOARD_PINS_RGB_MAIN_IO,
                     (uint32_t)failure,
                     20U + (uint32_t)STATUS_LED_STRIP_STATUS,
                     force_non_dma ? 1U : 0U);
            failed_strip_mask = (uint8_t)(
                failed_strip_mask | (strip_mask & STATUS_LED_STRIP_MASK_MAIN));
        }
    }
...
static uint8_t status_led_transmit_shutdown_final_latch_frame(
    const status_led_frame_t *frame,
    uint8_t strip_mask,
    bool key_dark_latch_pending_tx)
{
    ...
    if ((failed_strip_mask & STATUS_LED_STRIP_MASK_STATUS) == 0U) {
        const uint8_t non_status_mask =
            (uint8_t)(strip_mask & (uint8_t)~STATUS_LED_STRIP_MASK_STATUS);
        if (non_status_mask != 0U) {
            failed_strip_mask = (uint8_t)(
                failed_strip_mask |
...
static bool status_led_transition_clear_obsolete_for_shutdown_confirm(uint32_t started_ms_snapshot)
{
    bool obsolete = false;
    if (xSemaphoreTake(s_mutex, pdMS_TO_TICKS(STATUS_LED_TX_MUTEX_WAIT_MS)) == pdTRUE) {
        uint32_t now_ms = status_led_now_ms();
        obsolete = s_state.shutdown_confirm_started_ms != started_ms_snapshot &&
...
static void status_led_render_frame_locked(status_led_frame_t *frame, uint32_t now_ms)
{
    ...
    status_led_apply_boot_feedback_locked(frame, now_ms, &safety);
...
static void status_led_clear_stale_processing_locked(uint32_t now_ms)
{
    if (!s_state.processing_active || s_state.processing_started_ms == 0U) {
        return;
    }
    if (now_ms - s_state.processing_started_ms < STATUS_LED_PROCESSING_STALE_TIMEOUT_MS) {
        return;
    }
    ...
    status_led_set_last_reason_locked("processing_stale_timeout");
    diag_log(DIAG_SRC_STATUS_LED, DIAG_LED_STATE, DIAG_SEV_WARN,
             3, 0U, STATUS_LED_PROCESSING_STALE_TIMEOUT_MS, 0);
}
...
static uint32_t status_led_refresh_once(void)
{
    ...
    uint32_t shutdown_confirm_started_snapshot = 0U;
    ...
    shutdown_confirm_started_snapshot = s_state.shutdown_confirm_started_ms;
    status_led_copy_frame_locked(&frame);
    status_led_log_visual_state_locked(&frame, now_ms);
    xSemaphoreGive(s_mutex);

    if (force_clear_tx &&
        status_led_transition_clear_obsolete_for_shutdown_confirm(shutdown_confirm_started_snapshot)) {
...
static void status_led_resume_shutdown_confirm_output_locked(void)
{
    bool was_low_power_output = s_state.output_disabled || s_state.low_power_disabled;
    s_state.preview_suppress_accents = false;
    s_state.output_disabled = false;
...
{
    (void)status_led_strip_backend_suspend(s_strips[STATUS_LED_STRIP_STATUS].backend);
    (void)status_led_strip_backend_suspend(s_strips[STATUS_LED_STRIP_EDGE].backend);
    for (size_t index = 0; index < STATUS_LED_STRIP_COUNT; ++index) {
        s_strip_transport_suspended[index] = true;
    }
}
...
static void status_led_mark_boot_feedback_locked(uint32_t now_ms)
{
    ...
    s_state.boot_ble_ready_notified = false;
    s_state.boot_ble_ready_seen = false;
}

static bool status_led_boot_feedback_active_locked(uint32_t now_ms)
{
    return !s_state.boot_ble_ready_notified && now_ms < s_state.boot_feedback_until_ms;
}
...
static bool status_led_release_boot_feedback_for_ble_locked(
    uint32_t now_ms,
    const status_led_frame_t *frame)
{
    if (status_led_boot_feedback_active_locked(now_ms) &&
        s_state.boot_ble_ready_seen &&
        status_led_ble_state_completes_boot_feedback(s_state.ble_state) &&
        frame != NULL &&
        status_led_rgb_is_on(frame->status[STATUS_LED_SEM_BLE])) {
        s_state.boot_ble_ready_notified = true;
        return true;
    }
    return false;
}

static bool status_led_boot_ready_reason_releases_feedback(const char *reason)
{
    return reason != NULL &&
           (strcmp(reason, "advertising_skip_connected") == 0 ||
            strcmp(reason, "directed_advertising_started") == 0 ||
            strcmp(reason, "undirected_advertising_started") == 0);
}
    ...
    bool *ret_safety)
{
    ...
    if (status_led_release_boot_feedback_for_ble_locked(now_ms, frame)) {
        return;
    }
...
esp_err_t status_led_init(void)
{
    ...
    ESP_LOGI(
        TAG,
        "status LED init: profile=%s physical_routes=2 main_gpio=%d main_count=%u main_order=status_ec11_key edge_gpio=%d edge_count=%u vdd_led_enable=always_on_assumed",
        status_led_profile_name(s_state.profile),
        (int)BOARD_PINS_RGB_MAIN_IO,
        (unsigned)STATUS_LED_MAIN_COUNT,
        (int)BOARD_PINS_RGB_EDGE_IO,
        (unsigned)STATUS_LED_EDGE_COUNT);
    diag_log(DIAG_SRC_STATUS_LED, DIAG_LED_STATE, DIAG_SEV_INFO,
             (uint32_t)s_state.profile, (uint32_t)BOARD_PINS_RGB_MAIN_IO,
             (uint32_t)STATUS_LED_MAIN_COUNT, (uint32_t)BOARD_PINS_RGB_EDGE_IO);
...
void status_led_set_ble_state(status_led_ble_state_t state, bool confidence_window)
{
    ...
    if (xSemaphoreTake(s_mutex, portMAX_DELAY) == pdTRUE) {
        ...
        const bool routine_low_power_ble =
            ...
            !status_led_ble_state_attention_locked(state);
...
void status_led_note_ble_boot_ready(const char *reason)
{
    ...
    bool changed = false;
    if (xSemaphoreTake(s_mutex, portMAX_DELAY) == pdTRUE) {
        const bool releases_feedback =
            status_led_boot_ready_reason_releases_feedback(reason) ||
            status_led_ble_state_completes_boot_feedback(s_state.ble_state);
        if (status_led_boot_feedback_active_locked(now_ms) || !s_state.boot_ble_ready_notified) {
            ESP_LOGI(
                TAG,
                "boot BLE-ready note: reason=%s release=%u visible_release_pending=%u state=%u boot_ms_left=%" PRIu32,
                reason != NULL ? reason : "null",
                releases_feedback ? 1U : 0U,
                releases_feedback && status_led_boot_feedback_active_locked(now_ms) ? 1U : 0U,
                (unsigned)s_state.ble_state,
                status_led_boot_feedback_active_locked(now_ms) ? s_state.boot_feedback_until_ms - now_ms : 0U);
        }
        if (releases_feedback) {
            if (!s_state.boot_ble_ready_seen) {
                s_state.boot_ble_ready_seen = true;
                changed = status_led_boot_feedback_active_locked(now_ms);
            }
        }
        xSemaphoreGive(s_mutex);
    }
...
static bool status_led_prepare_shutdown_confirm_frame_locked(status_led_frame_t *frame, uint32_t now_ms)
{
    if (!status_led_shutdown_confirm_active_locked(now_ms)) {
        return false;
    }

    status_led_rgb_t previous_ec11[STATUS_LED_EC11_COUNT];
    memcpy(previous_ec11, s_state.last_frame.ec11, sizeof(previous_ec11));
    status_led_render_frame_locked(frame, now_ms);
    if (!s_state.shutdown_confirm_final &&
        !status_led_strip_has_light(frame->ec11, STATUS_LED_EC11_COUNT) &&
        status_led_strip_has_light(previous_ec11, STATUS_LED_EC11_COUNT)) {
        memcpy(frame->ec11, previous_ec11, sizeof(frame->ec11));
    }
    status_led_copy_frame_locked(frame);
    status_led_log_visual_state_locked(frame, now_ms);
    return true;
...
static void status_led_transmit_shutdown_confirm_frame(const status_led_frame_t *frame, bool final)
{
    const uint8_t write_count = final ? STATUS_LED_LOW_POWER_FINAL_LATCH_WRITES : 1U;
    const uint8_t pending_strip_mask =
        (!final && !status_led_strip_has_light(frame->ec11, STATUS_LED_EC11_COUNT))
            ? (uint8_t)(STATUS_LED_STRIP_MASK_ALL & (uint8_t)~STATUS_LED_STRIP_MASK_EC11)
            : (uint8_t)STATUS_LED_STRIP_MASK_ALL;
    uint8_t failed_strip_mask = 0U;
    for (uint8_t write_index = 0U; write_index < write_count; ++write_index) {
        failed_strip_mask = (uint8_t)(failed_strip_mask | (
            final
                ? status_led_transmit_shutdown_final_latch_frame(frame, STATUS_LED_STRIP_MASK_ALL, false)
                : status_led_transmit_changed_frame(frame, pending_strip_mask, false, false)));
    }
    status_led_update_retry_strip_mask(
        final ? STATUS_LED_STRIP_MASK_ALL : pending_strip_mask,
        failed_strip_mask);
}
...
{
    status_led_frame_t immediate_frame = {0};
    bool immediate_tx = false;
    bool changed = false;
    if (xSemaphoreTake(s_mutex, wait_ticks) == pdTRUE) {
        uint32_t now_ms = status_led_now_ms();
        status_led_resume_shutdown_confirm_output_locked();
        s_state.preview_effect_only = false;
        ...
        diag_log(DIAG_SRC_STATUS_LED, DIAG_LED_STATE, DIAG_SEV_INFO,
                 5, final ? 2U : 1U, 0, 0);
        immediate_tx = status_led_prepare_shutdown_confirm_frame_locked(&immediate_frame, now_ms);
        changed = true;
        xSemaphoreGive(s_mutex);
    }
    if (changed) {
        if (immediate_tx) {
            status_led_transmit_shutdown_confirm_frame(&immediate_frame, final);
        }
        status_led_request_refresh();
    }
    return changed;
...
static bool status_led_prepare_sleep_locked(bool *preserve_shutdown_final_hold)
{
    uint32_t now_ms = status_led_now_ms();
    *preserve_shutdown_final_hold =
        s_state.shutdown_confirm_final &&
        status_led_shutdown_confirm_active_locked(now_ms);
    if (!*preserve_shutdown_final_hold) {
        s_state.low_power_disabled = true;
        s_state.output_disabled = true;
    }
    s_state.test_mode = STATUS_LED_TEST_NONE;
...
{
    bool changed = false;
    bool preserve_shutdown_final_hold = false;
    if (s_mutex != NULL && xSemaphoreTake(s_mutex, portMAX_DELAY) == pdTRUE) {
        changed = status_led_prepare_sleep_locked(&preserve_shutdown_final_hold);
        xSemaphoreGive(s_mutex);
    }
    if (preserve_shutdown_final_hold) {
        return;
...
static void status_led_fill_print_snapshot_locked(status_led_print_snapshot_t *snapshot)
{
    ...
    snapshot->boot_feedback_until_ms = s_state.boot_feedback_until_ms;
    snapshot->boot_ble_ready_notified = s_state.boot_ble_ready_notified;
    snapshot->boot_ble_ready_seen = s_state.boot_ble_ready_seen;
...
static void status_led_print_status(void)
{
    ...
    if (xSemaphoreTake(s_mutex, portMAX_DELAY) == pdTRUE) {
        ...
        /* EC11 and key remain logical zones, but V2.2 clocks both through the
         * GPIO1 main backend. Mirror its physical transport data in diagnostic
         * readback so shared zones are not reported as unavailable. */
        for (size_t index = STATUS_LED_STRIP_EC11; index <= STATUS_LED_STRIP_KEY; ++index) {
            strip_gpios[index] = BOARD_PINS_RGB_MAIN_IO;
            strip_orders[index] = strip_orders[STATUS_LED_STRIP_STATUS];
            strip_available[index] = strip_available[STATUS_LED_STRIP_STATUS];
            strip_dma_requested[index] = strip_dma_requested[STATUS_LED_STRIP_STATUS];
            strip_dma[index] = strip_dma[STATUS_LED_STRIP_STATUS];
            strip_dma_fallback[index] = strip_dma_fallback[STATUS_LED_STRIP_STATUS];
            strip_transport_actual[index] = strip_transport_actual[STATUS_LED_STRIP_STATUS];
            strip_rmt_dma_requested[index] = strip_rmt_dma_requested[STATUS_LED_STRIP_STATUS];
            strip_rmt_dma[index] = strip_rmt_dma[STATUS_LED_STRIP_STATUS];
            strip_rmt_dma_fallback[index] = strip_rmt_dma_fallback[STATUS_LED_STRIP_STATUS];
            strip_spi_dma_requested[index] = 0U;
            strip_spi_dma[index] = 0U;
            strip_spi_dma_fallback[index] = 0U;
            strip_mem_block_symbols[index] = strip_mem_block_symbols[STATUS_LED_STRIP_STATUS];
        }
        strip_counts[STATUS_LED_STRIP_STATUS] = STATUS_LED_STATUS_COUNT;
        strip_counts[STATUS_LED_STRIP_EC11] = STATUS_LED_EC11_COUNT;
        strip_counts[STATUS_LED_STRIP_KEY] = STATUS_LED_KEY_COUNT;
    ...
    const bool boot_feedback_active =
        !snapshot.boot_ble_ready_notified && now_ms < snapshot.boot_feedback_until_ms;
    const uint32_t boot_feedback_ms_left =
        boot_feedback_active ? snapshot.boot_feedback_until_ms - now_ms : 0U;
    ...
    printf(
        ...
        " key_feedback_dynamic_tx=main_chain_rmt_dma key_release_fade_ms=%u key_single_white_hold_ms=%u"
        ...
        " separate_status_key_color_order=0 shared_main_color_order=GRB status_default_order=GRB key_default_order=GRB\n",
        STATUS_LED_REFRESH_MS,
        STATUS_LED_TASK_PRIORITY,
        STATUS_LED_TASK_CORE_ID,
        ...
        STATUS_LED_PROCESSING_STALE_TIMEOUT_MS);
    ...
    printf(
        ...
        " physical_routes=main:gpio1:count22:order_status_ec11_key,edge:gpio5:count6"
        " key_pin_contract=PWM_RGB_MAIN_GPIO1_SHARED ec11_pin_contract=PWM_RGB_MAIN_GPIO1_SHARED edge_pin_contract=PWM_RGB_Edge_GPIO5 gpio14_reserved=BAT_CHG_IO vdd_led_enable=always_on_assumed"
        "\n",
    ...
    printf(
        ...
        " boot_feedback_ms_left=%" PRIu32 " boot_ble_ready_seen=%u boot_ble_ready_notified=%u"
        " ble_repair_ms_left=%" PRIu32 " ble_repair_cue_ms_left=%" PRIu32
        ...
        boot_feedback_ms_left,
        snapshot.boot_ble_ready_seen ? 1U : 0U,
        snapshot.boot_ble_ready_notified ? 1U : 0U,
        ble_repair_ms_left,
        ble_repair_cue_ms_left,
...
static bool status_led_preview_is_shutdown_state(const char *state)
{
    return state != NULL &&
           (strcasecmp(state, "shutdown_confirm") == 0 ||
            strcasecmp(state, "power_hold") == 0 ||
            strcasecmp(state, "poweroff_confirm") == 0 ||
            strcasecmp(state, "shutdown_final") == 0 ||
...
static void status_led_preview_state(const char *state)
{
    ...
    const bool shutdown_preview = status_led_preview_is_shutdown_state(state);
    if (shutdown_preview) {
        s_state.transition_clear_mask = 0U;
    } else {
        status_led_schedule_idle_transition_clear_locked(now_ms);
    ...
    if (strcasecmp(state, "ready") == 0 || strcasecmp(state, "type_ready") == 0) {
        ...
        s_state.shutdown_final_all_zone_latched_started_ms = 0U;
               ...
               strcasecmp(state, "poweroff_final") == 0) {
        s_state.shutdown_confirm_started_ms = now_ms;
        s_state.shutdown_confirm_until_ms = STATUS_LED_SHUTDOWN_FINAL_HOLD_UNTIL_CANCEL_MS;
