...
把运行时预算视为验收设计的一部分，而不是可有可无的执行细节。

默认规则：

- 在运行任何 `build`、`flash`、抓音、回归、日志采集、主机侧或设备侧脚本前，先定义一个预期耗时预算
- 优先在命令执行时显式设置超时，而不是无上限等待
- 一旦超过该预算，默认应停止执行并按 `timeout` 报告，而不是继续死等
- 不能因为“命令还在跑”就默认推断它仍然健康
- 如果某一步确实需要比平时更长的超时，必须先写清楚为什么这个更长预算是合理的

推荐解释方式：

- `正常但有界`：允许在声明过的时间预算内运行
- `明显超预算`：直接终止，并把它视为需要解释的超时信号
- `确实需要更长预算`：先明确给出理由，再用更长超时重跑

报告超时时，不要只写 `timed out`。
...
除非已经有明确证据来自既往运行、仓库文档或实时进度日志，否则不要把明显偏长的等待合理化成“这大概正常”。
